...
Rust/Axum backend for the SaaS + AI billing platform.
...
```text
DATABASE_URL=postgres://app_user:app_password@127.0.0.1:5432/aisaas
...
Project-specific SaaS customization belongs under `src/modules/product`.
Keep shared capabilities in the core modules:

- `auth`, `iam`, `tenant`, `team` for admin identity and permissions.
- `customer`, `subscription`, `ai`, `server_api` for users, plans, wallet, usage, and AI gateway access.
- `web_assets`, `web_works`, `outbox`, `audit`, `notification`, `system` for product assets, works, background jobs, audit, notifications, and settings.

Legacy client authorization modules remain available for compatibility, but new SaaS product features should not be added to `license`, `device`, `client_auth`, `release`, or `secure_script` unless the task is explicitly maintaining that old surface.

`src/modules/product/defaults.rs` holds backend product defaults such as access mode, subscription plan codes, feature flags, wallet adjustment reasons, and AI job operation reasons. It is data-only and does not write to the database. If a concrete project needs one-click initialization, build that command or endpoint on top of these defaults and require explicit operator confirmation before mutating data.

`src/modules/product/seed_plan.rs` builds a serializable dry-run plan from those defaults. `GET /api/admin/product/seed-plan` exposes that plan for admins with `system:read`. The plan is the intended contract for future initialization commands or APIs, but it is still side-effect free.

## AI Billing Settlement

AI charging must use `src/modules/ai/billing.rs` as the single settlement boundary.
Both synchronous gateway calls and asynchronous generation jobs should follow the same flow:

```text
subscription check -> wallet check -> hold -> usage/job -> provider call -> asset cache -> capture/release/refund
```

When a project needs custom AI pricing, settlement, or refund rules, first extend this module and its tests, then adapt the gateway/job callers. Do not fork separate charge math inside `gateway.rs` or `jobs.rs`.
...
```bash
set DATABASE_URL=postgres://postgres@127.0.0.1:5432/aisaas_test
...
```powershell
$env:DATABASE_URL="postgres://postgres@127.0.0.1:5432/aisaas_test"
cargo test --test device_key_rotation -- --ignored
```
