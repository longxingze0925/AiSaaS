use std::sync::Arc;

use redis::{aio::MultiplexedConnection, Client as RedisClient};
use sqlx::PgPool;

use crate::{cache, config::AppConfig, db, error::AppError, storage};

#[derive(Clone)]
pub struct AppState {
    pub config: Arc<AppConfig>,
    pub db: PgPool,
    pub redis: RedisClient,
    pub redis_connection: Option<MultiplexedConnection>,
    pub object_store: Arc<dyn storage::ObjectStore>,
}

impl AppState {
    pub async fn connect(config: AppConfig) -> Result<Self, AppError> {
        let db = db::connect(&config.database).await?;
        let (redis, redis_connection) = cache::connect(&config.redis).await?;
        let object_store = storage::build_object_store(&config.object_storage)?;

        Ok(Self {
            config: Arc::new(config),
            db,
            redis,
            redis_connection: Some(redis_connection),
            object_store,
        })
    }

    pub async fn redis_connection(&self) -> redis::RedisResult<MultiplexedConnection> {
        match &self.redis_connection {
            Some(connection) => Ok(connection.clone()),
            None => self.redis.get_multiplexed_async_connection().await,
        }
    }
}
