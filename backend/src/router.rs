...
use crate::{
    ...
    modules::{
        ...
        outbox, platform, product, release, secure_script, server_api, subscription, system, team,
        tenant, web_api, web_assets, web_works,
    },
    state::AppState,
};
...
pub fn build(state: AppState) -> Router {
    ...
    let protected = Router::new()
        ...
        .route(
            ...
            post(ai::admin::test_ai_provider),
        )
        .route(
            "/api/admin/ai/models",
        ...
        .route(
            "/api/admin/ai/models/{id}/routes",
            get(ai::admin::list_ai_model_routes).post(ai::admin::create_ai_model_route),
        )
        .route(
            "/api/admin/ai/model-routes/{id}",
            put(ai::admin::update_ai_model_route).delete(ai::admin::delete_ai_model_route),
        )
        .route(
            "/api/admin/ai/model-routes/{id}/health/clear",
            post(ai::admin::clear_ai_model_route_health),
        )
        ...
        .route(
            ...
            get(product::admin::get_product_seed_plan),
        )
        .route(
            "/api/admin/product/seed-plan/execute",
            post(product::admin::execute_product_seed_plan),
        )
        .route(
            "/api/admin/product/seed-plan/executions",
            get(product::admin::list_product_seed_executions),
        )
        .route(
...
mod tests {
    ...
    fn test_state() -> AppState {
        let db = PgPoolOptions::new()
            .max_connections(1)
            .connect_lazy("postgres://postgres:postgres@127.0.0.1:1/aisaas_test")
    ...
    fn test_config() -> AppConfig {
        AppConfig {
            ...
            database: DatabaseConfig {
                url: "postgres://postgres:postgres@127.0.0.1:1/aisaas_test".to_owned(),
                max_connections: 1,
