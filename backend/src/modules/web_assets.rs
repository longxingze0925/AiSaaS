...
const UPLOAD_TOKEN_PREFIX: &str = "aisup_";
const LEGACY_UPLOAD_TOKEN_PREFIX: &str = "ehup_";
const UPLOAD_TOKEN_DISPLAY_LEN: usize = 18;
const UPLOAD_TOKEN_TTL_MINUTES: i64 = 15;
const UPLOAD_TOKEN_HEADER: &str = "x-aisaas-upload-token";
const LEGACY_UPLOAD_TOKEN_HEADER: &str = "x-entitlehub-upload-token";
...
pub async fn create_asset_upload_url(
    ...
    let upload = AssetUploadGrant {
        ...
        headers: json!({
            "X-AiSaaS-Upload-Token": plain_token,
...
fn upload_token_from_request(
    ...
    let token = headers
        ...
        .or_else(|| headers.get(LEGACY_UPLOAD_TOKEN_HEADER))
        .and_then(|value| value.to_str().ok())
        .or(query_token)
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(AppError::unauthenticated)?;
    if !upload_token_prefix_is_supported(token)
        || token.contains('\0')
...
fn upload_token_prefix_is_supported(token: &str) -> bool {
    token.starts_with(UPLOAD_TOKEN_PREFIX) || token.starts_with(LEGACY_UPLOAD_TOKEN_PREFIX)
}
