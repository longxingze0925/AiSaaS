...
use super::{
    billing::{settle_charge, settlement_metadata},
    route_health,
    runtime_routes::{
        load_enabled_model_routes, select_model_route_attempts, ResolvedAiModelRoute,
    },
};
...
struct JobModel {
    ...
    route_timeout_seconds: Option<i32>,
    route_retryable_statuses: Value,
    route_param_override: Value,
    route_header_override: Value,
    currency: String,
    billing_mode: String,
...
struct JobModelBase {
    id: Uuid,
    legacy_provider_id: Option<Uuid>,
    legacy_provider_kind: Option<String>,
    legacy_provider_name: Option<String>,
    legacy_provider_base_url: Option<String>,
    legacy_provider_config: Option<Value>,
    legacy_provider_secret_encrypted: Option<String>,
    code: String,
    modality: String,
...
#[derive(Debug)]
struct ProviderSubmitError {
    status: Option<reqwest::StatusCode>,
    provider_request_id: Option<String>,
    body: Option<Value>,
    message: String,
    retryable_without_status: bool,
}

struct ProviderSubmitSuccess {
    model: JobModel,
    submit: ProviderSubmitResult,
    route_attempts: Value,
}

struct ProviderSubmitFailure {
    model: JobModel,
    error: ProviderSubmitError,
    route_attempts: Value,
}

impl ProviderSubmitError {
    ...
    fn provider_status(
        ...
        Self {
            ...
            provider_request_id,
            body: Some(body),
            message: format!("ai provider submit failed: status {status}"),
            retryable_without_status: false,
        }
    }
...
async fn create_server_generation_job(
    ...
    let models = load_job_models(&state, server_key.tenant_id, model_code).await?;
    let model = models
        .first()
        .ok_or_else(|| AppError::not_found("ai model route not found or disabled"))?;
    ...
    let submit_result = submit_provider_generation_job_with_failover(
        &state,
        &models,
        job_type,
        &normalized_payload,
    ...
    let job = match submit_result {
        Ok(success) => {
            ...
            let job = insert_submitted_job(
                ...
                InsertJobInput {
                    ...
                    charge_mode: model.billing_mode.clone(),
                    quantity,
            ...
            .await?;
            if let Err(error) = update_generation_usage_route_metadata(
                &state,
                reservation.usage_id,
                &model,
                success.route_attempts,
            )
            .await
            {
                tracing::warn!(
                    %error,
                    usage_id = %reservation.usage_id,
                    "update ai generation usage route metadata failed"
                );
            }
        ...
        Err(failure) => {
            ...
            let provider_status = error
                ...
                .unwrap_or(0);
            let provider_request_id = error.provider_request_id.as_deref();
            let provider_body = error.body.as_ref();
            let app_error = AppError::dependency(error.message.clone());
            release_usage(
                &state,
                &reservation,
                &model,
...
async fn submit_provider_generation_job(
    ...
    job_type: &str,
    payload: Value,
) -> Result<ProviderSubmitResult, ProviderSubmitError> {
    match model.provider_kind.as_str() {
        "wuyin_keji" => submit_wuyin_generation_job(state, model, job_type, payload).await,
        _ => Err(ProviderSubmitError::dependency(
            "ai async generation provider is not supported",
        )),
    }
}

async fn submit_provider_generation_job_with_failover(
    state: &AppState,
    models: &[JobModel],
    job_type: &str,
    payload: &Value,
) -> Result<ProviderSubmitSuccess, ProviderSubmitFailure> {
    let mut attempts = Vec::with_capacity(models.len());

    for (index, model) in models.iter().enumerate() {
        let started = Instant::now();
        let result = submit_provider_generation_job(state, model, job_type, payload.clone()).await;
        let duration = started.elapsed();
        let has_next = index + 1 < models.len();

        match result {
            Ok(submit) => {
                route_health::record_route_success(state, model.route_id).await;
                let status = submit.http_status.as_u16();
                attempts.push(job_route_attempt_json(
                    model,
                    index,
                    Some(status),
                    None,
                ...
                return Ok(ProviderSubmitSuccess {
                    model: model.clone(),
                    submit,
                    route_attempts: Value::Array(attempts),
                });
            }
            Err(error) => {
                let status = error.status.map(|status| status.as_u16());
                let health_failure = job_submit_error_is_retryable(model, &error);
                let retryable = has_next && health_failure;
                if health_failure {
                    route_health::record_route_failure(state, model.route_id).await;
                }
                attempts.push(job_route_attempt_json(
                    model,
                    index,
                ...
                return Err(ProviderSubmitFailure {
                    model: model.clone(),
                    error,
                    route_attempts: Value::Array(attempts),
                });
    ...
    let model = models
        .first()
        .expect("ai generation provider failover requires at least one model")
        .clone();
    Err(ProviderSubmitFailure {
        model,
        error: ProviderSubmitError::dependency("ai generation provider route attempts are empty"),
        route_attempts: Value::Array(attempts),
    })
}
...
async fn submit_wuyin_generation_job(
    ...
    let secret = decrypt_provider_secret(state, model.provider_secret_encrypted.as_deref())
        .map_err(|error| ProviderSubmitError::dependency(error.to_string()))?;
    let mut headers = provider_headers(&secret, &model.provider_config)
        .map_err(|error| ProviderSubmitError::dependency(error.to_string()))?;
    append_configured_headers(&mut headers, Some(&model.route_header_override))
        .map_err(|error| ProviderSubmitError::dependency(error.to_string()))?;
    let timeout = provider_timeout_for_model(model)
        .map_err(|error| ProviderSubmitError::dependency(error.to_string()))?;
    let path = provider_submit_path(model, job_type)
        .map_err(|error| ProviderSubmitError::dependency(error.to_string()))?;
    let outbound_payload = provider_submit_payload(payload, model)
        .map_err(|error| ProviderSubmitError::dependency(error.to_string()))?;
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|error| {
            ProviderSubmitError::dependency(format!("ai provider client failed: {error}"))
        })?;
    ...
    .map_err(|error| {
        ProviderSubmitError::dependency(format!("ai provider request failed: {error}"))
    })?;
    let status = response.status();
    let provider_request_id = provider_request_id_from_headers(response.headers());
    let body = response_json(response).await.map_err(|error| {
        ProviderSubmitError::final_response(
            status,
    ...
    if !status.is_success() {
        return Err(ProviderSubmitError::provider_status(
            status,
            provider_request_id,
            body,
        ));
    }
    let provider_job_id = provider_job_id_from_body(&body).map_err(|error| {
        ProviderSubmitError::final_response(
            status,
            provider_request_id.clone(),
            Some(body.clone()),
...
fn wuyin_submit_payload(mut payload: Value, model: &JobModel) -> Result<Value, AppError> {
    ...
    match model_key.as_str() {
        ...
        "video_vidu" | "vidu" => {
            move_field(object, "ratio", "aspect_ratio");
            move_field(object, "aspectRatio", "aspect_ratio");
            let images = object.remove("images");
            ...
            merge_wuyin_urls(
                ...
                [images, image_url, reference_image_urls, first_frame_url],
            );
        }
        "video_omni" => {
            move_field(object, "ratio", "aspect_ratio");
            move_field(object, "aspectRatio", "aspect_ratio");
            move_field(object, "resolution", "mode");
            let image_list = object.remove("image_list");
            let image_url = object.remove("image_url");
            let video_list = object.remove("video_list");
            ...
            let refer_type = object.remove("refer_type");
            merge_wuyin_image_list(
                object,
                "image_list",
                [image_list, image_url, reference_image_urls],
                [first_frame, first_frame_url],
                [last_frame, last_frame_url],
            );
            merge_wuyin_video_list(
                object,
                "video_list",
                [video_list, video_url, reference_video_urls],
                refer_type,
            );
        }
        ...
        "grok_imagine" | "video_grok_imagine" => {
            move_field(object, "ratio", "aspect_ratio");
            move_field(object, "aspectRatio", "aspect_ratio");
            let image_url = object.remove("image_url");
            let image_urls = object.remove("image_urls");
            let first_frame = object.remove("firstFrameUrl");
            object.remove("lastFrameUrl");
            insert_first_string(
                object,
                "image_url",
                [
...
fn provider_submit_payload(payload: Value, model: &JobModel) -> Result<Value, AppError> {
    let mut payload = wuyin_submit_payload(payload, model)?;
    let object = payload
        .as_object_mut()
        .ok_or_else(|| AppError::validation_failed("ai generation body must be an object"))?;
    apply_json_object_overrides(object, &model.route_param_override)?;
    if let Some(provider_model) = model.provider_model.as_deref() {
...
fn apply_json_object_overrides(
    object: &mut Map<String, Value>,
    overrides: &Value,
) -> Result<(), AppError> {
    let Value::Object(overrides) = overrides else {
        return Err(AppError::validation_failed(
            "ai model route param override must be an object",
        ));
    };
...
fn job_submit_error_is_retryable(model: &JobModel, error: &ProviderSubmitError) -> bool {
    if let Some(status) = error.status {
        return job_status_is_retryable(model, status.as_u16());
    }

    error.retryable_without_status && job_error_is_retryable(model)
}

fn job_status_is_retryable(model: &JobModel, status: u16) -> bool {
    let Some(items) = model.route_retryable_statuses.as_array() else {
        return false;
    };
    ...
    items
        ...
        .any(|item| item.trim() == status)
}

fn job_error_is_retryable(model: &JobModel) -> bool {
    matches!(
        model.route_retryable_statuses.as_array(),
        Some(items) if !items.is_empty()
    )
}

fn job_route_attempt_json(
    model: &JobModel,
    index: usize,
    status: Option<u16>,
    error: Option<String>,
...
fn job_usage_metadata(model: &JobModel, mut metadata: Value, route_attempts: Value) -> Value {
    let Value::Object(object) = &mut metadata else {
        return metadata;
    };

    object.insert("model_route_id".to_owned(), json!(model.route_id));
    object.insert("provider_id".to_owned(), json!(model.provider_id));
    object.insert("provider_name".to_owned(), json!(&model.provider_name));
    if !value_array_is_empty(&route_attempts) {
        object.insert("route_attempts".to_owned(), route_attempts);
    }
...
fn merge_wuyin_image_list<const N: usize, const F: usize, const L: usize>(
    object: &mut Map<String, Value>,
    key: &str,
    reference_values: [Option<Value>; N],
    first_frame_values: [Option<Value>; F],
    last_frame_values: [Option<Value>; L],
) {
    let mut items = Vec::new();
    if let Some(first_value) = reference_values.first() {
        collect_wuyin_object_items(first_value.as_ref(), "image_url", &mut items);
    }
    push_wuyin_url_objects(reference_values, None, &mut items);
    push_wuyin_url_objects(first_frame_values, Some("first_frame"), &mut items);
    push_wuyin_url_objects(last_frame_values, Some("end_frame"), &mut items);
    if items.is_empty() {
        return;
    }
...
fn merge_wuyin_video_list<const N: usize>(
    object: &mut Map<String, Value>,
    key: &str,
    values: [Option<Value>; N],
    refer_type: Option<Value>,
) {
    let refer_type = value_to_string(refer_type.as_ref().unwrap_or(&Value::Null))
        .map(|value| value.trim().to_owned())
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| "feature".to_owned());
    let mut items = Vec::new();
    if let Some(first_value) = values.first() {
        collect_wuyin_object_items(first_value.as_ref(), "video_url", &mut items);
    }
    let mut urls = Vec::new();
    for value in values {
        collect_wuyin_input_urls(value.as_ref(), &mut urls);
...
fn collect_wuyin_object_items(value: Option<&Value>, url_key: &str, items: &mut Vec<Value>) {
    match value {
        Some(Value::Object(object)) if object.get(url_key).and_then(Value::as_str).is_some() => {
            items.push(Value::Object(object.clone()));
        }
        Some(Value::Array(values)) => {
            for value in values {
                collect_wuyin_object_items(Some(value), url_key, items);
            }
        }
        _ => {}
    }
...
fn push_wuyin_url_objects<const N: usize>(
    values: [Option<Value>; N],
    role: Option<&str>,
    items: &mut Vec<Value>,
) {
    ...
    for url in urls {
        let already_exists = items.iter().any(|item| {
            item.get("image_url")
                .and_then(Value::as_str)
                .is_some_and(|existing_url| existing_url == url)
        });
        if already_exists {
            continue;
        }
        let mut item = Map::new();
        item.insert("image_url".to_owned(), Value::String(url));
        if let Some(role) = role {
            item.insert("type".to_owned(), Value::String(role.to_owned()));
        }
...
async fn store_ai_asset(
    ...
    if let Some(app_id) = job.app_id {
        let input_metadata = job
            .request_payload
            .get("aisaas_input")
            .or_else(|| job.request_payload.get("entitlehub_input"))
            .cloned()
            .unwrap_or_else(|| json!({}));
...
async fn load_job_models(
    state: &AppState,
    tenant_id: Uuid,
    model_code: &str,
) -> Result<Vec<JobModel>, AppError> {
    let base = sqlx::query_as::<_, JobModelBase>(
        r#"
        select
          m.id,
          p.id as legacy_provider_id,
          p.kind as legacy_provider_kind,
          p.name as legacy_provider_name,
          p.base_url as legacy_provider_base_url,
          p.config_json as legacy_provider_config,
          p.secret_encrypted as legacy_provider_secret_encrypted,
          m.code,
          m.modality,
          m.provider_model as legacy_provider_model,
          m.currency,
          m.billing_mode,
    ...
    let routes = load_enabled_model_routes(state, tenant_id, base.id).await?;
    let route_attempts = select_model_route_attempts(&routes);
    if route_attempts.is_empty() {
        return Ok(vec![job_model_from_base(base, None)?]);
    }

    let models = route_attempts
        .into_iter()
        .map(|route| job_model_from_base(base.clone(), Some(route)))
        .collect::<Result<Vec<_>, _>>()?;

    Ok(filter_unhealthy_job_models(state, models).await)
}

async fn filter_unhealthy_job_models(state: &AppState, models: Vec<JobModel>) -> Vec<JobModel> {
    let route_ids = models
        .iter()
        .filter_map(|model| model.route_id)
        .collect::<Vec<_>>();
    let unhealthy_route_ids = route_health::unhealthy_route_ids(state, &route_ids).await;
    if unhealthy_route_ids.is_empty() {
        return models;
    }

    filter_job_models_by_unhealthy_ids(models, &unhealthy_route_ids)
}

fn filter_job_models_by_unhealthy_ids(
    models: Vec<JobModel>,
    unhealthy_route_ids: &HashSet<Uuid>,
) -> Vec<JobModel> {
    if unhealthy_route_ids.is_empty() {
        return models;
    }

    let healthy_models = models
        .iter()
        .filter(|model| {
            model
                .route_id
                .is_none_or(|route_id| !unhealthy_route_ids.contains(&route_id))
        })
        .cloned()
        .collect::<Vec<_>>();
...
fn job_model_from_base(
    base: JobModelBase,
    route: Option<ResolvedAiModelRoute>,
    ...
    if let Some(route) = route {
        return Ok(JobModel {
            ...
            provider_secret_encrypted: route.provider_secret_encrypted,
            code: base.code,
            modality: base.modality,
            provider_model: route.provider_model.or(base.legacy_provider_model),
            route_timeout_seconds: route.timeout_seconds,
            route_retryable_statuses: route.retryable_statuses,
            route_param_override: route.param_override,
            route_header_override: route.header_override,
            currency: base.currency,
            billing_mode: base.billing_mode,
            input_1k_price_minor: base.input_1k_price_minor,
            output_1k_price_minor: base.output_1k_price_minor,
            request_price_minor: base.request_price_minor,
            image_price_minor: base.image_price_minor,
            second_price_minor: base.second_price_minor,
            minute_price_minor: base.minute_price_minor,
            daily_spend_limit_minor: base.daily_spend_limit_minor,
            pricing_config: base.pricing_config,
        });
    }

    let provider_id = base
        .legacy_provider_id
        .ok_or_else(|| AppError::not_found("ai model route not found or disabled"))?;
    ...
    Ok(JobModel {
        ...
        provider_kind: base
            ...
            .ok_or_else(|| AppError::not_found("ai model provider not found or disabled"))?,
        provider_name: base
        ...
        provider_base_url: base
            ...
            .ok_or_else(|| AppError::not_found("ai model provider not found or disabled"))?,
        provider_config: base.legacy_provider_config.unwrap_or_else(|| json!({})),
        provider_secret_encrypted: base.legacy_provider_secret_encrypted,
        code: base.code,
        modality: base.modality,
        provider_model: base.legacy_provider_model,
        route_timeout_seconds: None,
        route_retryable_statuses: json!([]),
        route_param_override: json!({}),
        route_header_override: json!({}),
        currency: base.currency,
        billing_mode: base.billing_mode,
        input_1k_price_minor: base.input_1k_price_minor,
        output_1k_price_minor: base.output_1k_price_minor,
...
async fn release_usage(
    ...
    if reservation.held_minor > 0 {
        insert_wallet_ledger_entry(
            ...
            reason,
            reservation.usage_id,
            job_usage_metadata(model, json!({}), route_attempts.clone()),
        )
        .await?;
    }
    update_usage_failed(
        ...
        reservation.held_minor,
        provider_body,
        job_usage_metadata(model, json!({}), route_attempts),
    )
    .await?;
    transaction.commit().await.map_err(map_db_error)
}
...
async fn update_generation_usage_route_metadata(
    state: &AppState,
    usage_id: Uuid,
    model: &JobModel,
    route_attempts: Value,
    ...
    .bind(usage_id)
    .bind(model.provider_id)
    .bind(job_usage_metadata(model, json!({}), route_attempts))
    .execute(&state.db)
    .await
    .map(|_| ())
    .map_err(map_db_error)
    ...
    sqlx::query_as::<_, AiGenerationJobDetail>(
        ...
        select
          ...
          coalesce(
            ...
            case when coalesce(j.request_payload->'aisaas_input', j.request_payload->'entitlehub_input')->>'referenceCount' ~ '^[0-9]+$'
              then (coalesce(j.request_payload->'aisaas_input', j.request_payload->'entitlehub_input')->>'referenceCount')::bigint
            end,
            0
          ...
          coalesce(
            ...
            case when lower(coalesce(j.request_payload->'aisaas_input', j.request_payload->'entitlehub_input')->>'hasFirstFrame') in ('true', 'false')
              then (coalesce(j.request_payload->'aisaas_input', j.request_payload->'entitlehub_input')->>'hasFirstFrame')::boolean
            end,
            false
          ) as has_first_frame,
          coalesce(
            ...
            case when lower(coalesce(j.request_payload->'aisaas_input', j.request_payload->'entitlehub_input')->>'hasLastFrame') in ('true', 'false')
              then (coalesce(j.request_payload->'aisaas_input', j.request_payload->'entitlehub_input')->>'hasLastFrame')::boolean
            end,
            false
          ) as has_last_frame,
          work.visibility,
...
async fn load_and_validate_generation_input_assets(
    ...
    let mut reference_video_total_seconds = 0_i64;
    let mut reference_audio_total_seconds = 0_i64;
    ...
    for asset_id in &reference_asset_ids {
        ...
        if asset.asset_type.eq_ignore_ascii_case("video") {
            let duration_seconds =
                validate_reference_asset_duration(&asset, &capabilities, "referenceAssetIds")?;
            reference_video_count += 1;
            reference_video_total_seconds += duration_seconds.unwrap_or(0);
            reference_video_urls.push(url.clone());
        } else if asset.asset_type.eq_ignore_ascii_case("audio") {
            let duration_seconds =
                validate_reference_asset_duration(&asset, &capabilities, "referenceAssetIds")?;
            reference_audio_count += 1;
            reference_audio_total_seconds += duration_seconds.unwrap_or(0);
    ...
    if let Some(total_reference_video_seconds) = capabilities.total_reference_video_seconds {
        if reference_video_total_seconds > total_reference_video_seconds {
            return Err(AppError::validation_failed(format!(
                "reference_video_duration_too_long: reference videos total duration must be less than or equal to {total_reference_video_seconds} seconds"
            )));
        }
    }
    if let Some(total_reference_audio_seconds) = capabilities.total_reference_audio_seconds {
        if reference_audio_total_seconds > total_reference_audio_seconds {
            return Err(AppError::validation_failed(format!(
                "reference_audio_duration_too_long: reference audios total duration must be less than or equal to {total_reference_audio_seconds} seconds"
            )));
        }
    }
...
fn validate_asset_mime_and_size(
    ...
    if let (Some(max_mb), Some(file_size)) = (
        reference_asset_kind_size_limit(asset, capabilities),
        asset.file_size,
    ) {
        let max_bytes = max_mb.saturating_mul(1024 * 1024);
        if file_size > max_bytes {
...
fn reference_asset_kind_size_limit(
    asset: &web_assets::GenerationAssetRecord,
    capabilities: &capabilities::ModelCapabilities,
) -> Option<i64> {
    if asset.asset_type.eq_ignore_ascii_case("image") {
        capabilities.max_image_asset_size_mb
    } else if asset.asset_type.eq_ignore_ascii_case("video") {
        capabilities.max_video_asset_size_mb
    } else if asset.asset_type.eq_ignore_ascii_case("audio") {
        capabilities.max_audio_asset_size_mb
    } else {
        None
    }
}

fn validate_reference_asset_duration(
    asset: &web_assets::GenerationAssetRecord,
    capabilities: &capabilities::ModelCapabilities,
    field: &str,
) -> Result<Option<i64>, AppError> {
    let Some(duration_seconds) = reference_asset_duration_seconds(&asset.metadata) else {
        return Ok(None);
    };
    let (min_seconds, max_seconds, label) = if asset.asset_type.eq_ignore_ascii_case("video") {
        (
    ...
    if let Some(min_seconds) = min_seconds {
        if duration_seconds < min_seconds {
            return Err(AppError::validation_failed(format!(
                "reference_{label}_duration_too_short: {field} duration must be greater than or equal to {min_seconds} seconds"
            )));
        }
    }
    if let Some(max_seconds) = max_seconds {
        if duration_seconds > max_seconds {
            return Err(AppError::validation_failed(format!(
                "reference_{label}_duration_too_long: {field} duration must be less than or equal to {max_seconds} seconds"
            )));
        }
    }
...
fn reference_asset_duration_seconds(metadata: &Value) -> Option<i64> {
    for key in [
        "durationSec",
        "duration",
        "durationSeconds",
...
fn price_snapshot(model: &JobModel, held_minor: i64) -> Value {
    json!({
        ...
        "model_route_id": model.route_id,
        "route_retryable_statuses": &model.route_retryable_statuses,
...
fn provider_timeout_for_model(model: &JobModel) -> Result<u64, AppError> {
    match model.route_timeout_seconds {
        Some(timeout) => {
            if (1..=600).contains(&timeout) {
                Ok(timeout as u64)
            } else {
                Err(AppError::validation_failed(
                    "ai model route timeout is invalid",
                ))
            }
        }
        None => provider_timeout(&model.provider_config),
    }
}
...
mod tests {
    ...
    fn test_model(job_type: &str, provider_model: &str) -> JobModel {
        JobModel {
            ...
            code: job_type.to_owned(),
            modality: job_type.to_owned(),
            ...
            billing_mode: match job_type {
                "image" => "per_image",
                "audio" => "audio_per_request",
                _ => "video_per_second",
            }
            .to_owned(),
            ...
            image_price_minor: if job_type == "image" { 10 } else { 0 },
            second_price_minor: if job_type == "video" { 10 } else { 0 },
    ...
    #[test]
    fn job_retry_statuses_are_explicit() {
        let model = JobModel {
            route_retryable_statuses: json!(["429", "503"]),
            ..test_model("image", "image_gpt")
        };

        assert!(job_status_is_retryable(&model, 429));
        assert!(job_status_is_retryable(&model, 503));
        assert!(!job_status_is_retryable(&model, 400));
    }

    #[test]
    fn job_network_retry_requires_retryable_status_config() {
        assert!(!job_error_is_retryable(&test_model("image", "image_gpt")));

        let model = JobModel {
            route_retryable_statuses: json!(["503"]),
            ..test_model("image", "image_gpt")
        };
    ...
    #[test]
    fn job_usage_metadata_records_final_route_attempts() {
        ...
        let attempts = json!([{"index": 1, "status": 503, "retryable": true}]);
        let metadata = job_usage_metadata(&model, json!({"source": "server_key"}), attempts);

        assert_eq!(metadata["model_route_id"], json!(route_id));
        assert_eq!(metadata["provider_id"], json!(provider_id));
        assert_eq!(metadata["provider_name"], json!("fallback"));
        assert_eq!(metadata["route_attempts"][0]["status"], json!(503));
        assert_eq!(metadata["source"], json!("server_key"));
    }

    #[test]
    fn job_route_health_filter_skips_unhealthy_routes() {
        let first_route_id = uuid::Uuid::new_v4();
        let second_route_id = uuid::Uuid::new_v4();
        let first = JobModel {
            route_id: Some(first_route_id),
            provider_name: "unhealthy".to_owned(),
            ..test_model("image", "image_gpt")
        };
        let second = JobModel {
            route_id: Some(second_route_id),
            provider_name: "healthy".to_owned(),
            ..test_model("image", "image_gpt")
        };
        let unhealthy = HashSet::from([first_route_id]);

        let models = filter_job_models_by_unhealthy_ids(vec![first, second], &unhealthy);

        assert_eq!(models.len(), 1);
        assert_eq!(models[0].route_id, Some(second_route_id));
    }

    #[test]
    fn job_route_health_filter_fails_open_when_all_routes_unhealthy() {
        let first_route_id = uuid::Uuid::new_v4();
        let second_route_id = uuid::Uuid::new_v4();
        let first = JobModel {
            route_id: Some(first_route_id),
        ...
        let unhealthy = HashSet::from([first_route_id, second_route_id]);

        let models = filter_job_models_by_unhealthy_ids(vec![first, second], &unhealthy);

        assert_eq!(models.len(), 2);
        assert_eq!(models[0].route_id, Some(first_route_id));
        assert_eq!(models[1].route_id, Some(second_route_id));
    }
    ...
    #[test]
    fn wuyin_image_payload_maps_unified_ratio_and_resolution_fields() {
        let mut model = JobModel {
            ...
            provider_model: Some("NanoBanana2".to_owned()),
            route_id: None,
            route_timeout_seconds: None,
            route_retryable_statuses: json!([]),
            route_param_override: json!({}),
        ...
        assert_eq!(payload["aspectRatio"], json!("21:9"));
        assert_eq!(payload["size"], json!("4K"));
        assert_eq!(payload["urls"], json!(["https://cdn.example.com/ref.png"]));
        assert!(payload.get("resolution").is_none());
        assert!(payload.get("reference_image_urls").is_none());

        model.provider_model = Some("image_wan2.7".to_owned());
        let payload = wuyin_submit_payload(
            json!({
                "model": "image",
                "prompt": "test",
        ...
        assert_eq!(payload["size"], json!("1280*1280"));
        assert_eq!(payload["urls"], json!("https://cdn.example.com/ref.png"));
        assert!(payload.get("resolution").is_none());
    }

    #[test]
    fn vidu_payload_uses_official_q3_image_to_video_fields() {
        let model = test_model("video", "video_vidu");
        let payload = wuyin_submit_payload(
            json!({
                "model": "video",
                "prompt": "test",
        ...
        assert_eq!(payload["model"], json!("video_vidu"));
        assert_eq!(payload["aspect_ratio"], json!("16:9"));
        assert_eq!(payload["resolution"], json!("1080p"));
        assert_eq!(
            payload["images"],
            json!([
        ...
        assert!(payload.get("reference_image_urls").is_none());
        assert!(payload.get("reference_video_urls").is_none());
        assert!(payload.get("first_frame_url").is_none());
        assert!(payload.get("video_url").is_none());
    }

    #[test]
    fn kling_omni_payload_uses_image_and_video_lists() {
        let model = test_model("video", "video_omni");
        let payload = wuyin_submit_payload(
            json!({
                "model": "video",
                "prompt": "test",
        ...
        assert_eq!(payload["model"], json!("video_omni"));
        assert_eq!(payload["aspect_ratio"], json!("9:16"));
        assert_eq!(payload["mode"], json!("pro"));
        assert_eq!(
            payload["image_list"],
            json!([
                {"image_url": "https://cdn.example.com/ref.png"},
                {"image_url": "https://cdn.example.com/first.png", "type": "first_frame"},
                {"image_url": "https://cdn.example.com/last.png", "type": "end_frame"}
            ])
        );
        assert_eq!(
            payload["video_list"],
            json!([
                {"video_url": "https://cdn.example.com/ref.mp4", "refer_type": "base"}
            ])
        );
        assert!(payload.get("reference_image_urls").is_none());
        assert!(payload.get("reference_video_urls").is_none());
        assert!(payload.get("firstFrameUrl").is_none());
        assert!(payload.get("lastFrameUrl").is_none());
        assert!(payload.get("refer_type").is_none());
    }

    #[test]
    fn grok_video_payload_uses_single_image_url() {
        let model = test_model("video", "video_grok_imagine");
        let payload = wuyin_submit_payload(
            json!({
                "model": "video",
                "prompt": "test",
        ...
        assert_eq!(payload["model"], json!("video_grok_imagine"));
        assert_eq!(payload["aspect_ratio"], json!("16:9"));
        assert_eq!(
            payload["image_url"],
            json!("https://cdn.example.com/first.png")
        );
        assert!(payload.get("image_urls").is_none());
        assert!(payload.get("reference_image_urls").is_none());
        assert!(payload.get("first_frame_url").is_none());
        assert!(payload.get("last_frame_url").is_none());
    }
    ...
    #[test]
    fn grok_image_payload_keeps_image_urls_for_editing() {
        let model = test_model("image", "image_grok_imagine");
        let payload = wuyin_submit_payload(
            json!({
                "model": "image",
                "prompt": "test",
        ...
        assert_eq!(payload["model"], json!("image_grok_imagine"));
    ...
    fn normalized_generation_payload_removes_structured_reference_assets() {
        ...
        assert_eq!(payload["aisaas_input"]["referenceCount"], json!(1));
    }
