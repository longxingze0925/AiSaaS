...
async fn chat_completions_for_caller(
    ...
    let models = load_gateway_models(&state, caller.tenant_id, model_code).await?;
    let model = models
        .first()
    ...
    let provider_result =
        forward_openai_compatible_json_with_failover(&state, &models, &payload, "chat/completions")
            .await;
    ...
    match provider_result {
        Ok(success) => {
            ...
            let provider_response = success.response;
        ...
        Err(GatewayProviderFailure::ProviderResponse {
            model,
        ...
        Err(GatewayProviderFailure::Error {
            model,
            error,
            route_attempts,
        }) => {
...
async fn image_generations_for_caller(
    ...
    let provider_started = Instant::now();
    let provider_result = forward_openai_compatible_json_with_failover(
        &state,
        &models,
        &payload,
        "images/generations",
    ...
    match provider_result {
        Ok(success) => {
            let model = success.model;
            let mut provider_response = success.response;
...
async fn embeddings_for_caller(
    ...
    let provider_started = Instant::now();
    let provider_result =
        forward_openai_compatible_json_with_failover(&state, &models, &payload, "embeddings").await;
    metrics::record_ai_gateway_provider_duration(&endpoint, provider_started.elapsed());
...
async fn forward_openai_compatible_json_with_failover(
    state: &AppState,
    models: &[GatewayModel],
    payload: &Value,
    provider_path: &str,
) -> Result<GatewayProviderSuccess, GatewayProviderFailure> {
    let mut attempts = Vec::with_capacity(models.len());

    for (index, model) in models.iter().enumerate() {
        let started = Instant::now();
        let result =
            forward_openai_compatible_json(state, model, payload.clone(), provider_path).await;
        ...
        match result {
            Ok(response) if response.status.is_success() => {
                ...
                attempts.push(gateway_route_attempt_json(
                    model,
                    index,
                ...
                return Ok(GatewayProviderSuccess {
                    model: model.clone(),
                    response,
            ...
            Ok(response) => {
                let health_failure = gateway_status_is_retryable(model, response.status.as_u16());
                ...
                return Err(GatewayProviderFailure::ProviderResponse {
                    model: model.clone(),
                    response,
                    route_attempts: Value::Array(attempts),
                });
            ...
            Err(error) => {
                let health_failure = gateway_error_is_retryable(model);
                let retryable = has_next && health_failure;
                if health_failure {
                    route_health::record_route_failure(state, model.route_id).await;
                }
                ...
                return Err(GatewayProviderFailure::Error {
                    model: model.clone(),
                    error,
                    route_attempts: Value::Array(attempts),
                });
    ...
    let model = models
        .first()
        .expect("gateway provider failover requires at least one model")
        .clone();
    Err(GatewayProviderFailure::Error {
        model,
        error: AppError::dependency("ai provider route attempts are empty"),
        route_attempts: Value::Array(attempts),
    })
}
...
struct GatewayProviderSuccess {
    model: GatewayModel,
    response: ProviderResponse,
    route_attempts: Value,
}

enum GatewayProviderFailure {
    ProviderResponse {
        model: GatewayModel,
        response: ProviderResponse,
        route_attempts: Value,
...
async fn load_gateway_models(
    state: &AppState,
    tenant_id: Uuid,
    model_code: &str,
) -> Result<Vec<GatewayModel>, AppError> {
    let base = sqlx::query_as::<_, GatewayModelBase>(
        r#"
        select
          m.id,
    ...
    if route_attempts.is_empty() {
        return Ok(vec![gateway_model_from_base(base, None)?]);
    }

    let models = route_attempts
        .into_iter()
        .map(|route| gateway_model_from_base(base.clone(), Some(route)))
        .collect::<Result<Vec<_>, _>>()?;

    Ok(filter_unhealthy_gateway_models(state, models).await)
}

async fn filter_unhealthy_gateway_models(
    state: &AppState,
    ...
    filter_gateway_models_by_unhealthy_ids(models, &unhealthy_route_ids)
}

fn filter_gateway_models_by_unhealthy_ids(
    models: Vec<GatewayModel>,
...
fn gateway_model_from_base(
    base: GatewayModelBase,
...
async fn capture_usage(
    ...
    if settlement.captured_minor > 0 {
        let mut ledger_metadata = settlement_metadata(settlement);
        if let Some(metadata) = ledger_metadata.as_object_mut() {
            metadata.insert("model".to_owned(), json!(model.code));
        }
        insert_wallet_ledger_entry(
            &mut transaction,
            wallet.tenant_id,
    ...
    update_usage_succeeded(
        ...
        usage,
        settlement.captured_minor,
        gateway_usage_metadata(model, settlement_metadata(settlement), route_attempts),
        provider_body,
    )
    .await?;
    transaction.commit().await.map_err(map_db_error)
...
async fn release_usage(
    ...
    if reservation.held_minor > 0 {
        insert_wallet_ledger_entry(
            ...
            reason,
            reservation.usage_id,
            gateway_usage_metadata(model, json!({}), route_attempts.clone()),
        )
        .await?;
    }
    update_usage_failed(
        ...
        reservation.held_minor,
        provider_body,
        gateway_usage_metadata(model, json!({}), route_attempts),
    )
    .await?;
    transaction.commit().await.map_err(map_db_error)
}
...
fn gateway_status_is_retryable(model: &GatewayModel, status: u16) -> bool {
...
fn gateway_error_is_retryable(model: &GatewayModel) -> bool {
    matches!(
...
fn reqwest_header_name(value: &str) -> Result<ReqwestHeaderName, AppError> {
    ReqwestHeaderName::from_bytes(value.trim().as_bytes())
...
fn append_configured_headers(
    ...
    for (key, value) in map {
        ...
        let name = reqwest_header_name(key)?;
        if matches!(
            name.as_str(),
...
fn provider_timeout_for_model(model: &GatewayModel) -> Result<u64, AppError> {
...
fn gateway_json_response(status: StatusCode, body: Value, usage_id: Uuid) -> Response {
    ...
    if let Ok(value) = HeaderValue::from_str(&usage_id.to_string()) {
        response
            .headers_mut()
            .insert("x-aisaas-usage-id", value.clone());
        response
            .headers_mut()
...
mod tests {
    ...
    use crate::modules::{
        ai::{billing::settle_charge, capabilities},
    ...
    use super::{
        ...
        estimate_image_hold_minor, estimate_video_hold_minor, extension_for_mime,
        filter_gateway_models_by_unhealthy_ids, gateway_error_is_retryable,
        gateway_status_is_retryable, gateway_usage_metadata, idempotency_key, normalize_mime_type,
        provider_headers, provider_payload, provider_timeout, token_price_minor,
    ...
    #[test]
    fn provider_payload_applies_route_param_override_without_replacing_model() {
        let model = GatewayModel {
            route_param_override: json!({
                "temperature": 0.2,
                "model": "override-should-not-win"
            }),
            ..fixture_model()
        };
        let payload =
    ...
    #[test]
    fn provider_headers_keep_bearer_default_for_api_key() {
        let headers =
            provider_headers(&json!({"api_key": "sk-test"}), &json!({})).expect("headers");
    ...
    #[test]
    fn provider_headers_support_custom_api_key_header_and_scheme() {
        let headers = provider_headers(
            &json!({"api_key": "token"}),
            &json!({"api_key_header": "x-api-key", "auth_scheme": ""}),
        )
        .expect("headers");
    ...
    #[test]
    fn gateway_retry_statuses_are_explicit() {
        let model = GatewayModel {
            route_retryable_statuses: json!(["429", "503"]),
            ..fixture_model()
        };

        assert!(gateway_status_is_retryable(&model, 429));
        assert!(gateway_status_is_retryable(&model, 503));
        assert!(!gateway_status_is_retryable(&model, 400));
    }

    #[test]
    fn gateway_network_retry_requires_retryable_status_config() {
        assert!(!gateway_error_is_retryable(&fixture_model()));
        ...
        assert!(gateway_error_is_retryable(&model));
    }

    #[test]
    fn gateway_usage_metadata_records_final_route_attempts() {
        ...
        let metadata = gateway_usage_metadata(&model, json!({"captured_minor": 10}), attempts);
        ...
        assert_eq!(metadata["captured_minor"], json!(10));
    }

    #[test]
    fn gateway_route_health_filter_skips_unhealthy_routes() {
        let first_route_id = Uuid::new_v4();
        let second_route_id = Uuid::new_v4();
        let first = GatewayModel {
        ...
        let models =
            filter_gateway_models_by_unhealthy_ids(vec![first.clone(), second.clone()], &unhealthy);
    ...
    #[test]
    fn gateway_route_health_filter_fails_open_when_all_routes_unhealthy() {
        let first_route_id = Uuid::new_v4();
        let second_route_id = Uuid::new_v4();
        let first = GatewayModel {
            route_id: Some(first_route_id),
