...
use super::route_health::{self, RouteHealthSnapshot};
...
const MAX_PROVIDER_TEST_ERROR_LEN: usize = 500;
...
pub struct AiModelRoute {
    ...
    pub created_at: DateTime<Utc>,
    pub updated_at: DateTime<Utc>,
    pub health: RouteHealthSnapshot,
}

#[derive(Debug, Clone, FromRow)]
struct AiModelRouteRecord {
    id: Uuid,
    model_id: Uuid,
    provider_id: Uuid,
...
#[derive(Debug, Serialize)]
pub struct AiModelRouteListResponse {
    pub items: Vec<AiModelRoute>,
}

#[derive(Debug, Serialize)]
pub struct AiModelRouteResponse {
    pub route: AiModelRoute,
}
...
#[derive(Debug, Deserialize)]
pub struct CreateAiModelRouteRequest {
    pub provider_id: Uuid,
    pub provider_model: Option<String>,
    pub enabled: Option<bool>,
...
#[derive(Debug, Deserialize)]
pub struct UpdateAiModelRouteRequest {
    pub provider_id: Option<Uuid>,
    pub provider_model: Option<Option<String>>,
    pub enabled: Option<bool>,
    pub priority: Option<i32>,
    ...
    pub retryable_statuses: Option<Value>,
    pub param_override: Option<Value>,
    pub header_override: Option<Value>,
}
...
pub struct AiWalletListResponse {
    ...
    pub meta: AiWalletListMeta,
}

#[derive(Debug, Serialize)]
pub struct AiWalletListMeta {
    pub page: i64,
    pub page_size: i64,
    pub has_more: bool,
}
...
pub async fn test_ai_provider(
    ...
    let before = find_provider(&state, admin.tenant_id, provider_id).await?;
    let test_result = run_provider_dry_run_test(&state, &before);
    ...
    let mut transaction = state.db.begin().await.map_err(map_db_error)?;
    let record = update_provider_test_result_in_transaction(
        &mut transaction,
        admin.tenant_id,
        provider_id,
        status,
...
pub async fn list_ai_model_routes(
    State(state): State<AppState>,
    Extension(admin): Extension<AdminContext>,
    Extension(request_id): Extension<RequestId>,
    Path(model_id): Path<Uuid>,
) -> Result<Json<ApiResponse<AiModelRouteListResponse>>, AppError> {
    ensure_admin_permission(&admin, "ai:read")?;
    ensure_model_exists(&state, admin.tenant_id, model_id).await?;

    let mut items = list_model_routes(&state, admin.tenant_id, model_id).await?;
    attach_model_route_health(&state, &mut items).await;

    Ok(Json(ApiResponse::ok(
        AiModelRouteListResponse { items },
        request_id.to_string(),
    )))
}

pub async fn create_ai_model_route(
    State(state): State<AppState>,
    Extension(admin): Extension<AdminContext>,
    Extension(request_id): Extension<RequestId>,
    Path(model_id): Path<Uuid>,
    Json(payload): Json<CreateAiModelRouteRequest>,
) -> Result<Json<ApiResponse<AiModelRouteResponse>>, AppError> {
    ensure_admin_permission(&admin, "ai:model:update")?;
    let input = normalize_create_model_route_input(payload)?;

    let mut transaction = state.db.begin().await.map_err(map_db_error)?;
    ensure_model_exists_in_transaction(&mut transaction, admin.tenant_id, model_id).await?;
    ensure_provider_exists_in_transaction(
        &mut transaction,
        admin.tenant_id,
        Some(input.provider_id),
    )
    .await?;
    let route = insert_model_route(&mut transaction, admin.tenant_id, model_id, input).await?;
    audit::record(
        &mut transaction,
        AuditLogInput {
            tenant_id: Some(admin.tenant_id),
            actor_type: "team_member",
            actor_id: Some(admin.team_member_id),
            action: "ai_model_route.create",
            resource_type: "ai_model_route",
            resource_id: Some(route.id),
            ip: None,
            user_agent: None,
            request_id: Some(request_id.to_string()),
            before_json: None,
            after_json: Some(model_route_audit_json(&route)),
            metadata_json: json!({
    ...
    Ok(Json(ApiResponse::ok(
        AiModelRouteResponse { route },
        request_id.to_string(),
    )))
}

pub async fn update_ai_model_route(
    State(state): State<AppState>,
    Extension(admin): Extension<AdminContext>,
    Extension(request_id): Extension<RequestId>,
    Path(route_id): Path<Uuid>,
    Json(payload): Json<UpdateAiModelRouteRequest>,
) -> Result<Json<ApiResponse<AiModelRouteResponse>>, AppError> {
    ensure_admin_permission(&admin, "ai:model:update")?;

    let mut transaction = state.db.begin().await.map_err(map_db_error)?;
    let before = find_model_route_for_update(&mut transaction, admin.tenant_id, route_id).await?;
    let input = normalize_update_model_route_input(&before, payload)?;
    ensure_provider_exists_in_transaction(
        &mut transaction,
        admin.tenant_id,
        Some(input.provider_id),
    ...
    .await?;
    let route =
        update_model_route_in_transaction(&mut transaction, admin.tenant_id, route_id, input)
            .await?;
    audit::record(
        &mut transaction,
        AuditLogInput {
            ...
            actor_type: "team_member",
            actor_id: Some(admin.team_member_id),
            action: "ai_model_route.update",
            resource_type: "ai_model_route",
            resource_id: Some(route.id),
            ip: None,
            user_agent: None,
            request_id: Some(request_id.to_string()),
            before_json: Some(model_route_audit_json(&before)),
            after_json: Some(model_route_audit_json(&route)),
            metadata_json: json!({
                "model_id": route.model_id,
                "provider_id": route.provider_id,
    ...
    transaction.commit().await.map_err(map_db_error)?;
    let mut route = route;
    route.health = route_health::clear_route_health(&state, route.id).await;
...
pub async fn delete_ai_model_route(
    State(state): State<AppState>,
    Extension(admin): Extension<AdminContext>,
    Extension(request_id): Extension<RequestId>,
    Path(route_id): Path<Uuid>,
    ...
    let mut transaction = state.db.begin().await.map_err(map_db_error)?;
    let route = delete_model_route(&mut transaction, admin.tenant_id, route_id).await?;
    audit::record(
        &mut transaction,
        AuditLogInput {
            tenant_id: Some(admin.tenant_id),
            actor_type: "team_member",
            actor_id: Some(admin.team_member_id),
            action: "ai_model_route.delete",
            resource_type: "ai_model_route",
            resource_id: Some(route.id),
            ip: None,
            user_agent: None,
            request_id: Some(request_id.to_string()),
            before_json: Some(model_route_audit_json(&route)),
            after_json: None,
            metadata_json: json!({
                "model_id": route.model_id,
                "provider_id": route.provider_id,
    ...
    .await?;
    transaction.commit().await.map_err(map_db_error)?;
    route_health::clear_route_health(&state, route.id).await;
...
pub async fn clear_ai_model_route_health(
    State(state): State<AppState>,
    Extension(admin): Extension<AdminContext>,
    Extension(request_id): Extension<RequestId>,
    Path(route_id): Path<Uuid>,
    ...
    let mut transaction = state.db.begin().await.map_err(map_db_error)?;
    let route = find_model_route_for_update(&mut transaction, admin.tenant_id, route_id).await?;
    audit::record(
        &mut transaction,
        AuditLogInput {
            tenant_id: Some(admin.tenant_id),
            actor_type: "team_member",
            actor_id: Some(admin.team_member_id),
            action: "ai_model_route.health.clear",
            resource_type: "ai_model_route",
            resource_id: Some(route.id),
            ip: None,
            user_agent: None,
...
pub async fn list_ai_wallets(
    ...
    Ok(Json(ApiResponse::ok(
        AiWalletListResponse {
            items,
            meta: AiWalletListMeta {
                page,
                page_size,
                has_more,
            },
...
async fn update_provider_test_result_in_transaction(
    transaction: &mut Transaction<'_, Postgres>,
    tenant_id: Uuid,
    provider_id: Uuid,
    status: &str,
    ...
    .bind(provider_id)
    .bind(status)
    .bind(error.map(truncate_provider_test_error))
    .fetch_one(&mut **transaction)
    .await
    .map_err(map_db_error)
}
...
async fn list_model_routes(
    ...
    tenant_id: Uuid,
    model_id: Uuid,
) -> Result<Vec<AiModelRoute>, AppError> {
    sqlx::query_as::<_, AiModelRouteRecord>(
        r#"
        select
          r.id,
          r.model_id,
        ...
        where r.tenant_id = $1
          and r.model_id = $2
        order by r.enabled desc, r.priority asc, r.weight desc, r.created_at asc, r.id asc
        "#,
    )
    .bind(tenant_id)
    .bind(model_id)
    .fetch_all(&state.db)
    .await
    .map(|records| records.into_iter().map(AiModelRoute::from).collect())
    .map_err(map_db_error)
}

async fn attach_model_route_health(state: &AppState, routes: &mut [AiModelRoute]) {
    let route_ids = routes.iter().map(|route| route.id).collect::<Vec<_>>();
    let snapshots = route_health::route_health_snapshots(state, &route_ids)
        .await
        .into_iter()
        .collect::<std::collections::HashMap<_, _>>();
    for route in routes {
        route.health = snapshots.get(&route.id).cloned().unwrap_or_default();
    }
}
...
async fn insert_model_route(
    ...
    sqlx::query_as::<_, AiModelRouteRecord>(
        ...
        returning
          ...
          timeout_seconds,
          retryable_statuses,
          param_override_json as param_override,
          header_override_json as header_override,
          created_at,
          updated_at
        "#,
    )
    ...
    .bind(input.retryable_statuses)
    .bind(input.param_override)
    .bind(input.header_override)
    .fetch_one(&mut **transaction)
    .await
    .map(AiModelRoute::from)
    .map_err(map_db_error)
}

async fn find_model_route_for_update(
    transaction: &mut Transaction<'_, Postgres>,
    tenant_id: Uuid,
    route_id: Uuid,
) -> Result<AiModelRoute, AppError> {
    ...
    .fetch_optional(&mut **transaction)
    .await
    .map(|record| record.map(AiModelRoute::from))
    .map_err(map_db_error)?
    .ok_or_else(|| AppError::not_found("ai model route not found"))
}

async fn update_model_route_in_transaction(
    transaction: &mut Transaction<'_, Postgres>,
    tenant_id: Uuid,
    route_id: Uuid,
    ...
    sqlx::query_as::<_, AiModelRouteRecord>(
        ...
        set
          ...
          weight = $7,
          timeout_seconds = $8,
          retryable_statuses = $9,
          param_override_json = $10,
...
async fn delete_model_route(
    ...
    sqlx::query_as::<_, AiModelRouteRecord>(
        ...
        select
          ...
          d.weight,
          d.timeout_seconds,
          d.retryable_statuses,
          d.param_override_json as param_override,
          d.header_override_json as header_override,
          d.created_at,
          d.updated_at
        from deleted d
        join ai_providers p
          on p.tenant_id = d.tenant_id
         and p.id = d.provider_id
        "#,
    )
    .bind(tenant_id)
    .bind(route_id)
...
async fn ensure_model_exists(
    ...
    if exists {
        ...
        Err(AppError::not_found("ai model not found"))
    }
}

async fn ensure_model_exists_in_transaction(
    transaction: &mut Transaction<'_, Postgres>,
    tenant_id: Uuid,
    model_id: Uuid,
) -> Result<(), AppError> {
...
fn normalize_create_model_route_input(
    payload: CreateAiModelRouteRequest,
) -> Result<CreateModelRouteInput, AppError> {
    Ok(CreateModelRouteInput {
        provider_id: payload.provider_id,
        provider_model: payload
            .provider_model
            .as_deref()
            ...
            .transpose()?,
        enabled: payload.enabled.unwrap_or(true),
        priority: normalize_route_priority(payload.priority.unwrap_or(100))?,
        weight: normalize_route_weight(payload.weight.unwrap_or(100))?,
        timeout_seconds: match payload.timeout_seconds {
            Some(value) => Some(normalize_route_timeout(value)?),
            None => None,
        },
        retryable_statuses: normalize_retryable_statuses(payload.retryable_statuses)?,
        param_override: normalize_public_json(
            payload.param_override,
            "ai model route param override",
            MAX_CONFIG_BYTES,
        )?,
        header_override: normalize_public_json(
            payload.header_override,
            "ai model route header override",
            MAX_CONFIG_BYTES,
        )?,
    })
}

fn normalize_update_model_route_input(
    before: &AiModelRoute,
    payload: UpdateAiModelRouteRequest,
) -> Result<UpdateModelRouteInput, AppError> {
    Ok(UpdateModelRouteInput {
        provider_id: payload.provider_id.unwrap_or(before.provider_id),
        provider_model: match payload.provider_model {
            Some(Some(provider_model)) => Some(normalize_provider_model(&provider_model)?),
            Some(None) => None,
        ...
        priority: match payload.priority {
            Some(priority) => normalize_route_priority(priority)?,
            None => before.priority,
        },
        weight: match payload.weight {
            Some(weight) => normalize_route_weight(weight)?,
            None => before.weight,
        },
        timeout_seconds: match payload.timeout_seconds {
            Some(Some(timeout)) => Some(normalize_route_timeout(timeout)?),
            Some(None) => None,
            None => before.timeout_seconds,
        },
        retryable_statuses: match payload.retryable_statuses {
            Some(value) => normalize_retryable_statuses(value)?,
            None => before.retryable_statuses.clone(),
        },
        param_override: match payload.param_override {
            Some(value) => {
                normalize_public_json(value, "ai model route param override", MAX_CONFIG_BYTES)?
            }
            None => before.param_override.clone(),
        },
        header_override: match payload.header_override {
            Some(value) => {
                normalize_public_json(value, "ai model route header override", MAX_CONFIG_BYTES)?
            }
            None => before.header_override.clone(),
        },
    })
}
...
fn normalize_route_priority(value: i32) -> Result<i32, AppError> {
    if (0..=100_000).contains(&value) {
        Ok(value)
    } else {
        Err(AppError::validation_failed(
            "ai model route priority is invalid",
        ))
    }
}

fn normalize_route_weight(value: i32) -> Result<i32, AppError> {
    if (0..=100_000).contains(&value) {
        Ok(value)
    } else {
        Err(AppError::validation_failed(
            "ai model route weight is invalid",
        ))
    }
}

fn normalize_route_timeout(value: i32) -> Result<i32, AppError> {
...
fn normalize_retryable_statuses(value: Value) -> Result<Value, AppError> {
    let value = if value.is_null() { json!([]) } else { value };
    let Some(items) = value.as_array() else {
        return Err(AppError::validation_failed(
            "ai model route retryable statuses must be an array",
        ));
    };
    if items.len() > 32 {
        return Err(AppError::validation_failed(
            "ai model route retryable statuses is too large",
        ));
    }
    for item in items {
        let Some(status) = item.as_str().map(str::trim) else {
            return Err(AppError::validation_failed(
                "ai model route retryable statuses must be strings",
            ));
        };
        if status.is_empty() || status.len() > 64 || status.contains('\0') {
            return Err(AppError::validation_failed(
                "ai model route retryable status is invalid",
            ));
        }
    }
    ensure_json_size(
        "ai model route retryable statuses",
        &value,
        MAX_CONFIG_BYTES,
    )?;
...
fn run_provider_dry_run_test(
    state: &AppState,
    provider: &AiProviderRecord,
) -> Result<(), AppError> {
    if !provider.enabled {
        return Err(AppError::validation_failed("ai provider is disabled"));
    }
    normalize_provider_kind(&provider.kind)?;
    normalize_base_url(&provider.base_url)?;

    let encrypted_secret = provider
        .secret_encrypted
        .as_deref()
        .ok_or_else(|| AppError::validation_failed("ai provider secret is missing"))?;
...
fn validate_provider_secret(secret: &Value) -> Result<(), AppError> {
    ensure_json_object("ai provider secret", secret)?;
    let has_api_key = secret
        .get("api_key")
        .and_then(Value::as_str)
        .is_some_and(|value| !value.trim().is_empty());
    let has_authorization = secret
        .get("authorization")
        .and_then(Value::as_str)
        .is_some_and(|value| !value.trim().is_empty());
...
fn truncate_provider_test_error(error: &str) -> String {
    error.chars().take(MAX_PROVIDER_TEST_ERROR_LEN).collect()
}
...
fn model_route_audit_json(record: &AiModelRoute) -> Value {
    json!({
        "id": record.id,
        ...
        "retryable_statuses": &record.retryable_statuses,
...
impl From<AiModelRouteRecord> for AiModelRoute {
    fn from(record: AiModelRouteRecord) -> Self {
        Self {
            id: record.id,
            model_id: record.model_id,
            provider_id: record.provider_id,
            ...
            created_at: record.created_at,
            updated_at: record.updated_at,
            health: RouteHealthSnapshot::default(),
        }
    }
}
