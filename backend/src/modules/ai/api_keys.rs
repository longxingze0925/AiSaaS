...
const AI_API_KEY_PREFIX: &str = "aisai_";
const LEGACY_AI_API_KEY_PREFIX: &str = "ehai_";
...
pub async fn authenticate_api_key(
    ...
    if !ai_api_key_prefix_is_supported(token) {
...
fn ai_api_key_prefix_is_supported(token: &str) -> bool {
    token.starts_with(AI_API_KEY_PREFIX) || token.starts_with(LEGACY_AI_API_KEY_PREFIX)
}
...
mod tests {
    ...
    use super::{
        ai_api_key_prefix_is_supported, bearer_token, display_prefix, generate_ai_api_key,
        normalize_key_name,
    };
    ...
    fn generated_key_has_public_prefix() {
        ...
        assert!(key.starts_with("aisai_"));
        assert!(key.len() > 40);
        assert_eq!(display_prefix(&key).chars().count(), 18);
        assert!(ai_api_key_prefix_is_supported("ehai_legacy"));
    }
    ...
    fn bearer_token_reads_ai_authorization_header() {
        ...
        headers.insert(
            ...
            HeaderValue::from_static("Bearer aisai_test"),
        );

        assert_eq!(bearer_token(&headers).expect("bearer"), "aisai_test");
    }
}
