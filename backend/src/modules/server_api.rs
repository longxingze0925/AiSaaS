...
const LEGACY_SERVER_API_KEY_PREFIX: &str = "ehsk_";
const DISPLAY_PREFIX_LEN: usize = 18;
const MAX_KEY_NAME_LEN: usize = 128;
const CUSTOMER_ID_HEADER: &str = "x-aisaas-customer-id";
const LEGACY_CUSTOMER_ID_HEADER: &str = "x-entitlehub-customer-id";
...
pub async fn authenticate_server_key(
    ...
    if !server_api_key_prefix_is_supported(token) {
...
pub fn customer_id_from_headers(headers: &HeaderMap) -> Result<Uuid, AppError> {
    let value = headers
        ...
        .or_else(|| headers.get(LEGACY_CUSTOMER_ID_HEADER))
...
fn server_api_key_prefix_is_supported(token: &str) -> bool {
    token.starts_with(SERVER_API_KEY_PREFIX) || token.starts_with(LEGACY_SERVER_API_KEY_PREFIX)
}
...
mod tests {
    ...
    use super::{
        ...
        normalize_key_name, normalize_scopes, scopes_from_value,
        server_api_key_prefix_is_supported, CUSTOMER_ID_HEADER, LEGACY_CUSTOMER_ID_HEADER,
        SCOPE_AI_INVOKE,
    };
    ...
    fn generated_key_has_server_prefix() {
        ...
        assert!(server_api_key_prefix_is_supported("ehsk_legacy"));
    }
    ...
    fn bearer_token_reads_server_authorization_header() {
        ...
        headers.insert(
            ...
            HeaderValue::from_static("Bearer aissk_test"),
        );

        assert_eq!(bearer_token(&headers).expect("bearer"), "aissk_test");
    }
    ...
    fn customer_id_header_is_required_and_parsed() {
        ...
        headers.insert(
            LEGACY_CUSTOMER_ID_HEADER,
            HeaderValue::from_str(&customer_id.to_string()).expect("legacy header"),
        );
        assert_eq!(
            customer_id_from_headers(&headers).expect("legacy customer id"),
            customer_id
        );
