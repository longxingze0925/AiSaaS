...
import { productAccessConfig } from "../../product/access";
...
export function ApplicationsPage() {
  ...
  const serverApiKeyMutation = useMutation({
    mutationFn: (values: ServerApiKeyFormValues) => {
      ...
      return createServerApiKey({
        ...
        scopes: productAccessConfig.serverApiKeyDefaults.scopes
      });
    },
      ...
      render: (_, record) => (
        <Space>
          ...
          {canRotateKey ? (
            <ConfirmActionButton
              title="轮换密钥"
              description="轮换后会生成新的接入密钥和签名密钥，旧密钥进入历史状态。"
              buttonProps={{
                size: "small",
                icon: <KeyRound size={14} />
              }}
      ...
      render: (_, record) => (
        <Space size={6}>
          ...
          {record.status === "active" ? (
            <ConfirmActionButton
              title="吊销服务端 Key"
              description="吊销后，使用这个 Key 的服务端调用方将无法继续调用 AI 接口。"
              confirmText="吊销"
              okText="吊销"
  ...
  const openCreateServerApiKey = () => {
    ...
    serverApiKeyForm.setFieldsValue({
      name: productAccessConfig.serverApiKeyDefaults.name
    });
  ...
  return (
    <section className="workspace-page">
      <div className="page-heading">
        <div>
          <Typography.Title level={2}>API 接入</Typography.Title>
          <Typography.Text type="secondary">默认产品接入配置、服务端 Key 和签名公钥</Typography.Text>
        </div>
        <Space>
          <Input.Search
            allowClear
      ...
      >
        <Space direction="vertical" size={12} className="token-result">
          <Alert
            type="info"
            showIcon
            message="接入密钥只会在创建或轮换时显示一次；签名公钥可以公开复制给可信服务端用于验签。"
          />
          <Typography.Text type="secondary">接入 Key（app_key）</Typography.Text>
      ...
      >
        {keyTarget ? (
          <Alert
            type="info"
            showIcon
            message="可信服务端可以使用公开 JWKS 地址自动获取签名公钥。"
            description={
              <Typography.Text
      ...
      >
        <Space direction="vertical" size={12} className="settings-stack">
          <Alert
            type="info"
            showIcon
            message="服务端 Key 用于后端服务调用 `/api/server/ai/v1/*`，浏览器前端不应保存它。"
          />
      ...
      >
        <Space direction="vertical" size={12} className="settings-stack">
          ...
          >
            ...
            <Typography.Text type="secondary">
              当前 Key 固定绑定到这个接入配置，并仅允许服务端发起 AI 调用。
            </Typography.Text>
          </Form>
        </Space>
      </Modal>
  ...
  return (
    ...
    >
      <Form<ApplicationFormValues>
        ...
        initialValues={{
          auth_mode: productAccessConfig.applicationDefaults.authMode,
          heartbeat_interval_seconds:
            productAccessConfig.applicationDefaults.heartbeatIntervalSeconds,
          offline_tolerance_seconds:
            productAccessConfig.applicationDefaults.offlineToleranceSeconds,
          max_devices_default: productAccessConfig.applicationDefaults.seatLimit
        }}
      >
        <Form.Item
          name="name"
        ...
        <Form.Item name="max_devices_default" label="默认席位上限">
...
function scopeLabel(value: string): string {
  const option = productAccessConfig.serverApiScopeOptions.find((item) => item.value === value);
