...
export function CustomersPage() {
  ...
  const query = useInfiniteQuery({
    ...INFINITE_TABLE_QUERY_OPTIONS,
    queryKey: ["admin", "customers", keyword, status, includeHistory],
    queryFn: ({ pageParam }) =>
      listCustomers({
        keyword,
        status,
  ...
  useEffect(() => {
    return () => {
      trimInfiniteQueryCache(queryClient, ["admin", "customers"]);
    };
  }, [queryClient]);

  const refetchFirstCustomerPage = async () => {
    trimInfiniteQueryCache(queryClient, ["admin", "customers"]);
    await query.refetch();
  };
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await refetchFirstCustomerPage();
    }
  });
      ...
      render: (_, record) => (
        <Space>
          ...
          {canDisable && record.status !== "disabled" ? (
            <ConfirmActionButton
              title="禁用用户"
              description="禁用后会撤销该用户的活跃会话，用户将无法继续登录使用。"
              buttonProps={{
                size: "small",
                icon: <Ban size={14} />
              }}
          ...
          {canResetPassword ? (
            <ConfirmActionButton
              title="重置密码"
              description="系统会向用户邮箱发送密码重置邮件，已有会话可能在重置后失效。"
              buttonProps={{
                size: "small",
    ...
    <section className="workspace-page">
      <div className="page-heading">
        <div>
          ...
          <Typography.Text type="secondary">SaaS 用户、账号状态和重置操作</Typography.Text>
        </div>
        <Space>
          <Input.Search
            allowClear
          ...
          />
          <Button icon={<RefreshCw size={16} />} onClick={() => void refetchFirstCustomerPage()} />
          {canCreate ? (
            <Button
              type="primary"
              icon={<Plus size={16} />}
