...
React/Vite administration console for the SaaS + AI billing platform.
...
src/product    product-specific customization boundary
...
Concrete SaaS products should customize `src/product` first:

- `config.ts` controls product name, short name, brand mark, and headline copy.
- `access.ts` controls API access defaults, server API key scopes, and scope labels.
- `subscription.ts` controls default plan code, seat limit, plan presets, and feature options.
- `ai.ts` controls wallet adjustment presets, product AI model-template additions/overrides, and manual billing operation reasons.
- `menu.tsx` adds product-only menu entries.
- `routes.tsx` adds product-only protected routes.
- `/product/setup` shows a read-only initialization blueprint assembled from
  `src/product` and the backend dry-run seed plan API; it does not write database records.

Keep shared platform surfaces in the core app: users, subscriptions, wallet,
AI providers/models/jobs/usage/assets, API access, audit, team, roles, and
system settings.
