...
### 13.3 产品初始化 dry-run 计划

```http
GET /api/admin/product/seed-plan
```
...
- 只读接口，不创建接入配置、不生成服务端 Key、不写入套餐或模型商品。
- 返回 Product Layer 后端默认配置生成的 dry-run 计划，以及当前租户的只读执行前检查。
- `preflight.checks[].status` 可能为 `exists`、`missing`、`conflict`、`config_only`、`manual`。
- `conflict` 表示同一初始化目标已有记录但核心字段不一致，真正执行前必须人工确认。
- 真正初始化执行接口必须另行设计，并明确目标租户、环境和幂等/冲突策略。

### 13.4 执行产品初始化

```http
POST /api/admin/product/seed-plan/execute
```
...
```json
{
  "confirmation_phrase": "INITIALIZE_PRODUCT",
  "execution": {
    "result": {
      "created_application": {
        "id": "uuid",
        ...
        "signing_key": {
          "id": "uuid",
          "kid": "kid",
          "key_scope": "app_request",
          "alg": "EdDSA",
          "status": "active"
        }
      },
      "existing_application_id": null,
      "created_server_api_key": {
        "id": "uuid",
        "app_id": "uuid",
        "name": "影像生成生产 Key",
        "key_prefix": "aissk_xxx",
        "plain_key": "只返回一次",
        "scopes": ["ai:invoke"]
      },
      "existing_server_api_key_id": null,
      "skipped": [
        {
...
- 写库前必须通过 dry-run preflight；存在 `conflict` 时拒绝执行。
- 默认接入配置通过 Product defaults 的 `application_slug` 幂等识别，缺失时创建，已存在且字段一致时复用。
- 服务端 Key 缺失时创建；若已有 active Key 覆盖默认 scopes，则复用；若同名 Key scopes 不一致，则拒绝执行。
- `app_secret` 和 `plain_key` 只在新建时返回一次，审计日志不得记录明文。
- 套餐预设当前为人工核对项，AI 计费默认原因是配置项，执行接口不写入这些数据。

### 13.5 产品初始化执行历史

```http
GET /api/admin/product/seed-plan/executions?page=1&page_size=20
```
...
```json
{
  "items": [
    {
      "id": "audit-log-uuid",
      "actor_id": "team-member-uuid",
      "actor_email": "owner@example.com",
      "actor_name": "Owner",
      "request_id": "request-id",
      "created_application_id": "uuid",
      "existing_application_id": null,
      "created_server_api_key_id": "uuid",
      "existing_server_api_key_id": null,
      "skipped": [
        {
          "key": "subscription.plans",
...
- 只读接口，从当前租户 `audit_logs` 中读取 `product.seed.execute` 摘要。
- 不返回历史 `app_secret` 或 `plain_key` 明文。
- 只暴露 Product 初始化执行摘要，不替代通用审计日志查询。
...
AI 计费用于配置三方渠道、模型售价、客户余额和客户 API Key。客户端 AI 中转第一版开放 OpenAI 兼容的 `/v1/chat/completions`、`/v1/embeddings`、`/v1/images/generations` 和 `/v1/videos/generations`，调用方只接触 AiSaaS API Key，不暴露三方平台密钥。
  ...
  "items": [
    {
      ...
      "last_test_status": "success",
      "last_test_error": null,
      "last_test_at": "2026-07-08T08:00:00Z",
      "created_at": "...",
      "updated_at": "..."
    }
  ]
...
- `kind` 第一版支持 `openai_compatible`、`custom_http`、`claude`、`gemini`、`deepseek`、`image`、`video`、`wuyin_keji`。
- `config` 只能放非敏感配置。
...
- 当前为 dry-run 配置校验：检查渠道是否启用、`base_url` 是否有效、`kind` 是否支持、密钥是否能解密且包含 `api_key` 或 `authorization`。
- 不会向三方模型平台发送请求，不会产生三方扣费或生成任务。
- 校验结果会写回 `last_test_status`、`last_test_error`、`last_test_at`，并写入审计日志。
- 校验失败时接口返回错误，同时渠道列表会显示最近失败原因。
...
模型路由用于把一个对外模型代码映射到多个三方渠道，支持主备、权重分流、超时和非敏感请求覆盖。旧字段 `ai_models.provider_id` / `provider_model` 继续兼容，迁移会回填一条默认路由。

```http
GET /api/admin/ai/models/{id}/routes
POST /api/admin/ai/models/{id}/routes
PUT /api/admin/ai/model-routes/{id}
DELETE /api/admin/ai/model-routes/{id}
```

认证：后台 session；写接口需要 CSRF。

权限：

```text
GET: ai:read
POST/PUT/DELETE: ai:model:update
```
  ...
  "items": [
    {
      ...
      "health": {
        ...
        "open": false,
        "failure_count": 0,
        "failure_ttl_seconds": null,
        "open_ttl_seconds": null
      },
      "created_at": "...",
      "updated_at": "..."
    }
...
- `priority` 数字越小优先级越高；首次尝试会在最小 `priority` 候选中按 `weight` 加权选择。
- `timeout_seconds` 为空表示继承渠道或网关默认超时。
- `retryable_statuses` 控制故障转移状态码。只有命中这里配置的 HTTP 状态码时才会继续尝试下一条路由；网络/请求异常只有在当前路由配置了可重试状态时才会继续尝试下一条路由。
- `param_override` / `header_override` 必须是普通 JSON 对象，只能保存非敏感覆盖项，不能包含密钥、token、Authorization 等字段。
- 网关运行时会优先使用启用的模型路由；首次选中路由失败且允许重试时，会按候选列表继续尝试其它启用路由。没有启用路由时回退 `ai_models.provider_id` / `provider_model` 旧字段。
- 路由健康状态使用 Redis 做短期熔断：同一路由 60 秒内累计 3 次可重试失败会临时跳过 120 秒；成功请求会清除该路由失败计数。Redis 异常不会阻断 AI 请求；如果所有候选路由都处于熔断状态，网关会 fail-open 继续尝试候选，避免完全不可用。
- 同步网关和异步生成任务提交都会在同一个 `usage_id` 内记录最终 `model_route_id`、`provider_id`、`provider_name` 和 `route_attempts`。不会因为故障转移创建多条用量记录或重复预扣。
- 已提交的异步任务继续按任务保存的 `provider_id` 轮询，不受后续路由调整影响。

手动清除路由健康状态：

```http
POST /api/admin/ai/model-routes/{id}/health/clear
```

权限：`ai:model:update`。返回清除后的路由对象，`health.status` 正常情况下为 `healthy`；如果 Redis 不可用则为 `unknown`。
...
- 扣减后余额不能小于冻结金额。
- 后续网关发起请求时使用 `hold` 预扣，三方成功后 `capture` 结算，失败、取消或人工标记失败时 `release` 释放预扣；已成功扣费后的人工退款使用 `refund`。

### 18.7.0 AI 计费闭环

同步 AI 网关和异步生成任务共用同一套计费闭环：

```text
订阅校验 -> 钱包检查 -> hold 预扣 -> 创建 usage/job -> 三方调用
  -> 成功缓存素材后 capture
  -> 失败/取消/人工失败 release
  -> 已扣费人工退款 refund
```
...
- `Idempotency-Key` 用于防止调用方重试造成重复预扣或重复扣费。
- `price_snapshot_json` 保存本次请求的价格快照、预扣金额和结算元数据，历史账单不受后续模型价格调整影响。
- `capture` 可能小于预扣金额，多余部分会 `release`；如果实际价格高于预扣，系统只会在当前钱包可用余额允许的范围内补扣，并在结算元数据记录 `shortfall_minor`。
- `timeout_review` 任务不会自动退款，必须由后台人工确认后走 `retry-poll`、`fail-release` 或其他处理动作。
- 已经 `capture` 的成功任务不能走 `fail-release`，只能走 `refund`。
  ...
  "items": [
    {
      ...
      "customer_name": "User",
      "name": "生产环境 SDK Key",
      "key_prefix": "aisai_xxxxxxxxxxxxx",
      "status": "active",
      "expires_at": null,
      "daily_spend_limit_minor": null,
      "last_used_at": "...",
...
Server Key 用于 Web 后端、业务服务端调用 AiSaaS。它绑定到一个应用，不绑定单个客户；调用时由业务后端通过请求头传入本次实际消费的客户 ID。浏览器和移动端页面不能保存或直连使用 Server Key。
...
```json
{
  "server_api_key": {
    ...
    "app_key": "app_xxx",
    "name": "影织 Web 后端",
    "key_prefix": "aissk_xxxxxxxxxxxxx",
    "status": "active",
    "scopes": ["ai:invoke"],
    "expires_at": null,
    "last_used_at": null,
...
- `X-AiSaaS-Customer-Id` 是本次消费的客户 ID。
...
- 低层图片/视频异步平台使用 `/images/jobs` 和 `/videos/jobs`：AiSaaS 创建自己的任务、冻结余额、提交三方任务、后台轮询三方结果、成功后缓存素材并确认扣费，三方失败则释放预扣金额。
...
给影织这类 Web SaaS 后端使用。浏览器仍然只调用影织后端，影织后端用 Server Key 调 AiSaaS。
...
- 注册/登录只校验 AiSaaS 客户账号；Web 浏览器 session 由影织后端自己维护。
- `GET /ai/models` 不需要客户 ID，用于渲染模型商品、比例、分辨率、时长、价格等前端选项。
- `POST /ai/jobs` 必须在请求体传 `customer_id`，AiSaaS 以该客户做订阅校验、钱包预扣和任务归属。
- `cancel` 是 AiSaaS 本地取消：停止继续轮询并释放未扣费预扣；如果三方平台没有取消接口，不能保证三方侧停止生成。
...
- AiSaaS 会校验素材状态、类型、MIME、大小和模型能力。
...
```text
管理/查询接口：Authorization: Bearer aissk_...
上传字节接口：X-AiSaaS-Upload-Token: aisup_...
```
...
- 资产状态包括 `uploading`、`processing`、`ready`、`failed`、`deleted`。用户上传视频会先返回 `processing`，AiSaaS 后台用后端镜像内置的 `ffprobe/ffmpeg` 解析真实时长并抽取封面，成功后转为 `ready`，失败后转为 `failed` 并写入 `metadata.processingError`。
- AI 生成成功后，如果第三方结果带封面或时长，AiSaaS 会写入资产 metadata 并在资产接口返回；用户上传视频会由异步媒体处理器补封面和真实时长。
- 参考素材提交给第三方时会使用资产 `public_url`。生产环境应使用第三方可访问的对象存储公开 URL 或短期签名 URL；如果 `public_url` 是需要 Server Key 的 AiSaaS 下载接口，第三方平台通常无法直接拉取。
...
- 图片/视频生成成功后，AiSaaS 自动创建 `Work`，默认 `visibility=private`。
...
异步任务用于速创等“提交后返回任务 ID、再查询结果”的图片/视频/音频平台。任务状态以第三方查询接口返回为准，平台负责内部商业状态：余额冻结、扣费/释放、素材缓存、后台审计。Web SaaS 产品优先使用统一入口 `/api/server/web/v1/ai/jobs`；低层入口保留给非 Web 内部服务。
...
- `caching`：三方已成功，AiSaaS 正在缓存图片/视频。
- `succeeded`：生成成功、素材已缓存、余额已确认扣费。
- `provider_failed` / `failed`：三方失败或不可恢复错误，预扣金额已释放。
- `timeout_review`：长时间没有最终结果，后台待人工确认；系统不会自行猜测失败。
- `cancelled`：业务侧取消了未完成任务，未结算预扣已释放。

速创接入建议：

- AI 渠道类型选择 `速创平台`，`base_url` 填 `https://api.wuyinkeji.com`。
- 渠道密钥填三方 API Key；OpenAI 兼容同步网关默认使用 `Authorization: Bearer <api_key>`，速创异步任务默认使用 `Authorization: <api_key>`。如三方要求其他 header 或认证前缀，可在渠道配置中设置 `api_key_header`、`auth_scheme` 或 `headers`。
...
- 速创字段适配会按模型能力拆分统一素材字段，不会把不支持的参考素材强行透传给三方。AiSaaS 会把统一的 `referenceAssets` / `referenceAssetIds` / `firstFrameAssetId` / `lastFrameAssetId` 转成三方要求的 `images`、`image_url`、`video_url`、`audio_url`、`image_list`、`video_list`、`urls`、`image_urls` 等字段。
- `video_seedance` 按火山方舟官方 Seedance 视频生成教程配置：支持 `text`、`image`、`frames`、`video`、`audio`；首帧图生视频 1 张，首尾帧 2 张，多模态参考图 1-9 张；最多 3 个参考视频，单个视频 2-15 秒、所有视频总时长不超过 15 秒、单个视频不超过 50 MB；最多 3 段参考音频，单段音频 2-15 秒、所有音频总时长不超过 15 秒、单个音频不超过 15 MB；图片单张不超过 30 MB，请求体不超过 64 MB。模型能力建议配置 `inputModes: ["text", "image", "frames", "video", "audio"]`、`maxReferenceImages: 9`、`maxReferenceVideos: 3`、`maxReferenceAudios: 3`、`supportsReferenceVideo: true`、`supportsReferenceAudio: true`、`supportsFirstFrame: true`、`supportsLastFrame: true`、`maxImageAssetSizeMb: 30`、`maxVideoAssetSizeMb: 50`、`maxAudioAssetSizeMb: 15`、`minReferenceVideoSeconds: 2`、`maxReferenceVideoSeconds: 15`、`totalReferenceVideoSeconds: 15`、`minReferenceAudioSeconds: 2`、`maxReferenceAudioSeconds: 15`、`totalReferenceAudioSeconds: 15`。
- `video_vidu` 当前按 Vidu Q3 图生视频落地：支持 `text`、`image`；参考图最多 1 张，提交给速创时使用 `images` 数组；支持 `540p`、`720p`、`1080p`，时长 `1-16` 秒；不声明参考视频和尾帧能力。Vidu reference-to-video 的多主体能力建议后续单独建模型商品，不和图生视频混在一个 `provider_model` 里。
- `video_omni` 按可灵 Omni 落地：支持 `text`、`image`、`frames`、`video`；比例 `16:9`、`9:16`、`1:1`；`mode` 使用 `std`、`pro`、`4k`；图片参考会转成 `image_list`，首帧/尾帧分别标记 `type: "first_frame"` / `type: "end_frame"`；视频参考会转成 `video_list`，默认 `refer_type: "feature"`。图片单张建议不超过 10 MB，视频单个不超过 200 MB且不短于 3 秒。
- `video_grok_imagine` 按 xAI Grok Imagine 视频能力保守落地：支持 `text`、`image`；图生视频只取 1 张图片作为 `image_url`；支持 `480p`、`720p`，最长 15 秒；不声明参考视频和尾帧能力。`image_grok_imagine` 图片编辑保留 `image_urls`，参考图最多 3 张。
- AiSaaS 会按模型能力校验参考素材数量、素材类型、MIME、文件大小；视频/音频资产如果已有 `durationSec`、`durationSeconds`、`duration` 或 `duration_seconds` 元数据，还会校验单个时长和总时长。第三方请求体总大小目前无法在引用 URL 模式下精确计算，直传/Base64 场景应由调用侧控制不超过上游限制。
- 速创图片模型能力基线：
  - `GPT-Image-2`：比例 `auto`、`1:1`、`3:2`、`2:3`、`16:9`、`9:16`、`4:3`、`3:4`、`21:9`、`9:21`、`1:3`、`3:1`、`2:1`、`1:2`；最多 9 张参考图。
  - `NanoBanana2`、`NanoBanana Pro`、`NanoBanana`：比例 `auto`、`1:1`、`16:9`、`9:16`、`4:3`、`3:4`、`3:2`、`2:3`、`5:4`、`4:5`、`21:9`；最多 14 张参考图。`NanoBanana2` 和 `NanoBanana Pro` 支持 `1K`、`2K`、`4K`，`NanoBanana` 支持 `1K`。
  - `Wan2.7 图片`：支持参考图最多 4 张；分辨率使用 `1280*1280`、`1104*1472`、`1472*1104`、`960*1696`、`1696*960`，AiSaaS 会把统一 `resolution` 转成速创 `size`。
- 以上参考图上限包含速创文档未写全但已由业务实测确认的能力；Web 产品应以 AiSaaS 返回的 `capabilities` 为准，不要写死。
...
返回体里的图片会被缓存到 AiSaaS 对象存储。三方返回 `data[].url` 时会替换成平台自有地址；三方返回 `data[].b64_json` 时会写入对象存储，并改为返回平台自有 `url`。
...
返回体里的视频 URL 会被缓存到 AiSaaS 对象存储。第一版识别并替换响应 JSON 中的 `url`、`video_url`、`output_url`、`download_url` 字段，替换后的值为平台自有 `/api/ai/assets/{id}` 地址。
...
该地址由图片/视频生成网关返回，用于下发已缓存到 AiSaaS 对象存储的生成素材。接口返回原始文件字节，并设置 `Content-Type`、`Content-Length` 和长期缓存头。
...
```http
GET /api/admin/ai/usage-records?status=succeeded&customer_query=user@example.com&provider_id=uuid&model_code=gpt-4o-mini&endpoint=/v1/chat/completions&request_id=req_xxx&provider_request_id=chatcmpl_xxx&page=1&page_size=50
```
...
- `status`：调用状态，支持 `pending`、`running`、`succeeded`、`failed`、`refunded`。
- `customer_id`：精确客户 ID。
- `customer_query`：客户邮箱、名称模糊搜索，或精确客户 ID。
- `provider_id`：精确渠道 ID。
- `model_code`：精确模型代码。
- `endpoint`：精确网关 endpoint。
- `request_id`：精确平台请求 ID。
- `provider_request_id`：精确三方请求 ID。
- `page` / `page_size`：分页参数。
...
说明：`metadata.model_route_id` / `provider_id` / `provider_name` 表示最终命中的模型路由和渠道；`metadata.route_attempts` 表示本次调用的路由尝试链路，后台可用它排查故障转移、三方状态和耗时。

### 18.17 AI 网关客户端接入示例

客户侧只需要 AiSaaS AI API Key，不需要三方平台密钥。后台把三方渠道、模型和价格配置好后，客户把 OpenAI 兼容 SDK 的 `base_url` / `baseURL` 指向 AiSaaS 即可。
...
- 响应头 `x-aisaas-usage-id` 可用于客户侧日志和后台调用记录排查。
