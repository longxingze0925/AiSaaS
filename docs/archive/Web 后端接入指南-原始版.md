...
本文给 Web 产品、SaaS 后端、业务服务端接入 AiSaaS AI 网关使用。浏览器端不要保存 AiSaaS Server Key，也不要直接请求第三方 AI 平台。
...
- 浏览器只登录你的业务系统，不保存 AiSaaS Server Key。
- 你的业务后端持有 AiSaaS Server Key。
- 每次调用都把实际消费的 AiSaaS 客户 ID 传给 AiSaaS。
- AiSaaS 负责订阅校验、客户状态校验、AI 钱包余额校验、预扣、扣费、失败释放、素材缓存和审计。
- 第三方平台状态以第三方查询接口返回为准，AiSaaS 不猜测任务成功或失败。
...
1. 你的业务后端鉴权当前 Web 用户。
2. 找到该 Web 用户对应的 AiSaaS `customer_id`。
3. 用 Server Key 请求 AiSaaS。
4. AiSaaS 检查应用、客户、订阅、钱包、限额。
5. AiSaaS 预扣余额。
6. AiSaaS 请求第三方平台。
...
影织这类 Web 产品建议优先使用这一组接口。它们仍然只允许业务后端调用，浏览器不要直接请求 AiSaaS。
...
- AiSaaS 只校验客户账号密码并返回客户身份。
- 影织自己的登录态、Cookie、JWT、Session 仍由影织后端自己维护。
- 不返回 AiSaaS access token，不走 `/api/client/auth/*` 设备授权体系。
...
- `model` 使用 AiSaaS 模型代码，不是第三方真实模型名。
- `customer_id` 是本次扣费、订阅校验和任务归属的 AiSaaS 客户 ID。
- AiSaaS 会按模型商品配置校验参数并预扣余额。
...
- AiSaaS 会按模型 `capabilities` 校验输入方式、参考素材数量、MIME 类型、大小、是否支持首帧/尾帧。
...
- `cancel` 是 AiSaaS 本地取消：停止继续轮询并释放未扣费预扣金额。
- 如果第三方平台没有取消接口，AiSaaS 不能保证第三方侧也停止生成。
...
```json
{
  ...
  "data": {
    "job": {
      ...
      "results": [{}],
      "asset_urls": [
        "https://aisaas.example.com/api/ai/assets/uuid"
      ],
      "assetUrls": [
        "https://aisaas.example.com/api/ai/assets/uuid"
      ],
      "assets": [
        {
          ...
          "asset_type": "video",
          "status": "ready",
          "url": "https://aisaas.example.com/api/server/web/v1/assets/uuid/download",
          "public_url": "https://aisaas.example.com/api/server/web/v1/assets/uuid/download",
          "mimeType": "video/mp4",
          "mime_type": "video/mp4",
...
| `processing` | 文件已上传，AiSaaS 正在解析视频、生成封面和时长 | 展示“处理中”，轮询资产详情 |
...
- AiSaaS 后台会用后端镜像内置的 `ffprobe` 读取真实时长、分辨率、编码信息，用 `ffmpeg` 抽取视频帧生成封面。
...
- AI 生成成功后，如果第三方结果里带封面或时长，AiSaaS 会写入资产 metadata 并在资产列表返回。
...
```json
{
  "upload": {
    "upload_id": "uuid",
    "method": "PUT",
    "url": "https://aisaas.example.com/api/server/web/v1/assets/uploads/{upload_id}",
    "upload_token": "aisup_...",
...
- 浏览器可以直接上传到 AiSaaS，但 Server Key 仍然只保存在影织后端。
...
```json
{
  "asset": {
    ...
    "mime_type": "image/png",
    "mimeType": "image/png",
    "thumbnailUrl": "https://aisaas.example.com/api/server/web/v1/assets/uuid/download",
    "file_size": 123456
  },
  "assetId": "uuid",
  "url": "https://aisaas.example.com/api/server/web/v1/assets/uuid/download",
...
- AiSaaS 会按渠道适配参考素材字段。速创 `google_omni` 会把参考图和参考视频合并成三方要求的 `images`，并把 `resolution` 转成 `size`。速创该接口没有明确首帧/尾帧角色时，不应把普通参考图当成确定首尾帧能力。
- 已内置速创图片、视频、音频模型路径映射：
...
- `video_seedance` 按火山方舟官方 Seedance 视频生成教程配置：支持 `text`、`image`、`frames`、`video`、`audio`；首帧图生视频 1 张，首尾帧 2 张，多模态参考图 1-9 张；最多 3 个参考视频，单个视频 2-15 秒、所有视频总时长不超过 15 秒、单个视频不超过 50 MB；最多 3 段参考音频，单段音频 2-15 秒、所有音频总时长不超过 15 秒、单个音频不超过 15 MB；图片单张不超过 30 MB，请求体不超过 64 MB。
...
- `/api/server/web/v1/ai/models` 会返回 `maxImageAssetSizeMb`、`maxVideoAssetSizeMb`、`maxAudioAssetSizeMb`、`minReferenceVideoSeconds`、`maxReferenceVideoSeconds`、`totalReferenceVideoSeconds`、`minReferenceAudioSeconds`、`maxReferenceAudioSeconds`、`totalReferenceAudioSeconds` 等能力字段。影织应优先按这些字段限制上传和提交；AiSaaS 后端也会校验参考素材数量、类型、MIME、文件大小，以及资产元数据里已有的视频/音频时长。
- 速创图片模型能力基线：
...
- 以上参考图上限包含速创文档未写全但已由业务实测确认的能力；影织应以 AiSaaS `/api/server/web/v1/ai/models` 返回的 `capabilities` 为准。
- 默认 JSON 提交；需要表单提交的模型可在模型价格配置或渠道配置里设置 `request_format: "form"`。
- 三方平台必须能访问这些 URL。生产环境建议使用可外网访问的对象存储公开 URL 或短期签名 URL。
- 如果使用本地文件存储，`public_url` 默认是 AiSaaS 下载接口，下载接口需要 Server Key，很多第三方平台无法直接拉取。
...
```json
{
  "work": {
    ...
    "primary_asset_url": "https://aisaas.example.com/api/ai/assets/...",
    "cover_asset_id": null,
...
```json
{
  "downloadUrl": "https://aisaas.example.com/api/ai/assets/asset-id",
...
- 下载状态也是客户维度关系，AiSaaS 会记录 `downloadedAt` 和下载次数，换设备后仍能展示。
...
这一节是 AiSaaS 低层 Server AI 接口，保留给非 Web 产品直接接入。影织这类 Web 产品优先使用 3A 的统一接口：`POST /api/server/web/v1/ai/jobs`、`GET /api/server/web/v1/ai/jobs`、`GET /api/server/web/v1/ai/jobs/{jobId}`。
...
| `running` | 三方生成中 | 继续轮询 |
| `caching` | 三方成功，AiSaaS 正在缓存素材 | 继续轮询 |
...
第三方成功后，AiSaaS 会下载第三方图片/视频并缓存，最终返回自己的素材 URL：

```json
{
  "asset_urls": [
    "https://aisaas.example.com/api/ai/assets/asset-id"
  ]
}
```

业务后端和浏览器只使用 AiSaaS 返回的 URL，不要使用第三方原始 URL。
...
async function createVideoJob(input: {
  ...
  const response = await fetch(`${input.aiSaasBaseUrl}/api/server/web/v1/ai/jobs`, {
    method: "POST",
    headers: {
...
async function getGenerationJob(input: {
  ...
  const response = await fetch(`${input.aiSaasBaseUrl}/api/server/web/v1/ai/jobs/${input.jobId}?customer_id=${input.customerId}`, {
    headers: {
      "Authorization": `Bearer ${input.serverKey}`
    }
  });
...
AiSaaS 负责平台级校验和钱包扣费，但你的业务后端仍要防止自己的业务入口被刷。
...
- 后台处理不会修改第三方平台状态，只修改 AiSaaS 内部商业状态。
...
- 业务后端保存 AiSaaS `job.id`，不要只保存三方任务 ID。
- 任务轮询有退避策略，不要高频打满。
- 客户可见素材 URL 使用 AiSaaS 返回的 `asset_urls`。
