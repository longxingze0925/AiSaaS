...
APP_NAME="AiSaaS"
PROJECT_NAME="${AISAAS_PROJECT_NAME:-aisaas}"
INSTALL_DIR="${AISAAS_INSTALL_DIR:-/opt/aisaas}"
...
AISAAS_ARCHIVE_URL="${AISAAS_ARCHIVE_URL:-https://github.com/${AISAAS_REPO}/archive/refs/heads/${AISAAS_REF}.tar.gz}"

ENV_FILE=".env.compose"
STATE_FILE=".install-state"
BACKUP_DIR="backups/installer"
DIGEST_FILE="${AISAAS_DIGEST_FILE:-compose.digests.yaml}"
PIN_DIGESTS="${AISAAS_PIN_DIGESTS:-1}"
DEPLOY_MODE="${AISAAS_DEPLOY_MODE:-image}"
...
preflight() {
  ...
  case "$DEPLOY_MODE" in
    image | source) ;;
    *) die "AISAAS_DEPLOY_MODE 只能是 image 或 source。" ;;
  esac
  install_docker_prompt
...
fetch_source() {
  ...
  log "下载最新源码包"
  curl -fsSL "$AISAAS_ARCHIVE_URL" | tar -xz --strip-components=1 -C "$dest"
}
...
run_migrations() {
  ...
  compose_base run --rm backend aisaas-backend migrate
}
...
pin_image_digests() {
  ...
  tmp_file="$(mktemp)"
  {
    printf '# Generated by ops/aisaasctl.sh. Do not edit by hand.\n'
    printf 'services:\n'
  } > "$tmp_file"
...
init_owner() {
  ...
  if [[ -n "$domain" ]]; then
    ...
    owner_email="admin@aisaas.local"
  fi
...
install_local_command() {
  if [[ -w /usr/local/bin || "${EUID:-$(id -u)}" -eq 0 ]]; then
    ...
    cat > /usr/local/bin/aisaas <<EOF
#!/usr/bin/env bash
set -Eeuo pipefail
export AISAAS_INSTALL_DIR="${INSTALL_DIR}"
export AISAAS_PROJECT_NAME="${PROJECT_NAME}"
export AISAAS_REPO="${AISAAS_REPO}"
export AISAAS_REF="${AISAAS_REF}"
export AISAAS_RAW_BASE="${AISAAS_RAW_BASE}"
export AISAAS_DEPLOY_MODE="\${AISAAS_DEPLOY_MODE:-${DEPLOY_MODE}}"
bash <(curl -fsSL "\$AISAAS_RAW_BASE/ops/install.sh")
EOF
    chmod +x /usr/local/bin/aisaas
    ln -sf /usr/local/bin/aisaas /usr/local/bin/aisaas
  fi
}
...
backup_flow() {
  ...
  target="$BACKUP_DIR/aisaas_${ts}.dump"
...
uninstall_flow() {
  ...
  case "$choice" in
    1)
      compose_base down
      rm -f /usr/local/bin/aisaas /usr/local/bin/aisaas
      printf '服务已停止，数据卷和 %s 已保留。\n' "$INSTALL_DIR"
      ;;
    2)
      printf '请输入 DELETE AISAAS DATA 确认清除：'
      local phrase
      read -r phrase
      [[ "$phrase" == "DELETE AISAAS DATA" ]] || die "确认短语不匹配。"
...
reset_admin_password_flow() {
  ...
  compose_base "${args[@]}" backend aisaas-backend reset-admin-password
}
