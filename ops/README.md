...
By default, the installer pulls prebuilt AiSaaS images:

```text
ghcr.io/longxingze0925/aisaas-backend:main
ghcr.io/longxingze0925/aisaas-admin:main
```

Set `AISAAS_DEPLOY_MODE=source` before running the installer only when you need the old server-side source build fallback.
...
```bash
cd /opt/aisaas
docker compose -p aisaas --env-file .env.compose -f compose.yaml run --rm \
  -e RESET_ADMIN_EMAIL=admin@example.com \
  backend aisaas-backend reset-admin-password
```
...
```bash
docker compose -p aisaas --env-file .env.compose up -d postgres redis
```
...
```bash
cd backend
DATABASE_URL=postgres://app_user:password@127.0.0.1:5432/aisaas sqlx migrate run
```

Or run embedded migrations through the backend container:

```bash
docker compose -p aisaas --env-file .env.compose run --rm backend aisaas-backend migrate
```

Then start the application stack:

```bash
docker compose -p aisaas --env-file .env.compose up -d --build
```
...
Run the AI gateway smoke with a real AiSaaS AI API Key:

```powershell
$env:AI_GATEWAY_API_KEY = "aisai_..."
...
The one-command installer automatically generates `compose.digests.yaml` during install, update, and certificate mode changes. In default image mode it pulls the current tag-based image set first, writes immutable `@sha256:` overrides, and then starts the stack with `compose.yaml` plus `compose.digests.yaml`. Set `AISAAS_PIN_DIGESTS=0` only for emergency registry troubleshooting. Digest pinning is skipped in `AISAAS_DEPLOY_MODE=source`.
...
```bash
docker compose -p aisaas --env-file .env.compose run --rm backend aisaas-backend init-owner
```
...
```bash
DATABASE_URL=postgres://app_user:password@127.0.0.1:5432/aisaas ./ops/backup-postgres.sh
```
...
```bash
DATABASE_URL=postgres://app_user:password@127.0.0.1:5432/aisaas BACKUP_FILE=./backups/aisaas_20260605T120000Z.dump ./ops/restore-postgres.sh
```
