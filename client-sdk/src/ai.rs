...
pub const USAGE_ID_HEADER: &str = "x-aisaas-usage-id";
pub const LEGACY_USAGE_ID_HEADER: &str = "x-entitlehub-usage-id";
...
pub fn usage_id_from_headers<'a>(headers: &'a [(String, String)]) -> Option<&'a str> {
    headers.iter().find_map(|(name, value)| {
        (name.eq_ignore_ascii_case(USAGE_ID_HEADER)
            || name.eq_ignore_ascii_case(LEGACY_USAGE_ID_HEADER))
...
mod tests {
    ...
    fn usage_id_and_image_urls_are_extracted_from_transport_data() {
        ...
        assert_eq!(
            usage_id_from_headers(&legacy_headers),
