...
Client AI calls use the signed client routes and return raw provider-compatible JSON on success. Failed AiSaaS checks can still return the normal backend error envelope.
...
let chat = AiGatewayJsonResponse::from_json_with_usage_id(
    &chat_response_body,
    response_headers.get("x-aisaas-usage-id"),
)?;
...
The backend handles balance checks, subscription gating, provider forwarding, and image/video caching. For image responses, `ai::image_urls_from_response` extracts URLs after the backend has replaced provider assets with AiSaaS asset URLs. For video responses, `ai::video_urls_from_response` extracts cached AiSaaS asset URLs from common provider response shapes.

For Web products, do not put AiSaaS Server Keys in the browser. Keep the Server Key in your business backend and call the server-side async generation routes:
...
Those routes are designed for async providers such as image/video task APIs. AiSaaS reserves wallet balance when the job is created, polls the provider result, caches generated assets under `/api/ai/assets/{id}`, and only captures the charge after the assets are ready.
